...
Notez que vous devez inclure la seule adresse SIP mais également le nom d’utilisateur et le mot de passe. Si vous n’incluez pas le mot de passe jeu-CsTestUserCredential vous invite à entrer ces informations. Le nom d’utilisateur peut être spécifié en utilisant le\\format de nom d’utilisateur nom de domaine indiqué ci-dessus ou en utilisant le nom d’utilisateur @ nom de domaine. par exemple:
