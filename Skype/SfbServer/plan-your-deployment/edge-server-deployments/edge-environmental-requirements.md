...
Une grande partie de la planification et de la préparation doit avoir lieu en dehors de l’environnement de serveur Edge Skype Entreprise Server lui-même. Dans cet article, nous examinerons les préparations qui doivent être réalisées dans l’environnement organisationnel, comme dans notre liste ci-dessous :
...
Les topologies de serveur Edge Skype Entreprise Server peuvent utiliser :
...
- Vous devez configurer la nat **symétrique** pour le trafic entrant et sortant. La nat symétrique est la seule nat prise en charge que vous pouvez utiliser avec le serveur Edge Skype Entreprise Server.
...
Vous n’aurez aucun problème à avoir vos interfaces Edge Access, de conférence web et A/V derrière un routeur ou un pare-feu qui exécutent une nat symétrique pour les topologies de serveur Edge consolidées à la fois unique et mise à l’échelle (tant que vous n’utilisez pas l’équilibrage de la charge matérielle).
  
### <a name="summary-of-edge-server-topology-options"></a>Résumé des options de topologie du serveur Edge

Nous avons plusieurs options de topologie disponibles pour les déploiements de serveur Edge Skype Entreprise Server :
...
|**Topologie**|**Disponibilité élevée**|**Enregistrements DNS supplémentaires requis pour le serveur Edge externe dans le pool edge ?**|**Le failover Edge pour les sessions Skype Entreprise Server**|**Le failover Edge pour les sessions de fédération Skype Entreprise Server**|
|:-----|:-----|:-----|:-----|:-----|
...
- **Équilibrage de la charge matérielle**: nous recommandons l’équilibrage de charge DNS pour la plupart des scénarios. L’équilibrage de la charge matérielle est également pris en charge, bien entendu, mais il est notamment nécessaire pour un scénario unique sur l’équilibrage de charge DNS :
  ...
  - Accès externe à la messagerie unifiée Exchange 2007 ou Exchange 2010 (sans SP).
    
- Équilibrage de charge **DNS**: pour la um, Exchange 2010 SP1 et les plus nouveaux peuvent être pris en charge par l’équilibrage de charge DNS. Notez que si vous devez passer à l’équilibrage de charge DNS pour une version antérieure d’Exchange, cela fonctionne, mais tout le trafic pour cela sera vers le premier serveur du pool, et s’il n’est pas disponible, ce trafic échouera par la suite.
...
Il s’agit des enregistrements DNS dont vous aurez besoin pour un serveur Edge en utilisant des IP publiques ou privées avec nat. Étant donné qu’il s’agit d’exemples de données, nous allons donner des exemples d’IP pour vous aider à trouver vos propres entrées plus facilement :
...
|DNS externe  <br/> |enregistrement SRV  <br/> |443  <br/> |_sip._tls.contoso.com  <br/> |sip.contoso.com  <br/> |Une interface externe pour votre service Edge d’accès. Cet enregistrement SRV est requis pour que les clients Skype Entreprise Server, Lync Server 2013 et Lync Server 2010 fonctionnent en externe. Vous en aurez besoin pour chaque domaine avec des utilisateurs Skype Entreprise.  <br/> |
|DNS externe  <br/> |enregistrement SRV  <br/> |5061  <br/> |_sipfederationtls._tcp.contoso.com  <br/> |sip.contoso.com  <br/> |Une interface externe pour votre service Edge d’accès. Cet enregistrement SRV est requis pour la découverte DNS automatique des partenaires fédérés appelés domaines SIP autorisés. Vous en aurez besoin pour chaque domaine avec des utilisateurs Skype Entreprise.  <br/> |
...
|DNS externe  <br/> |enregistrement SRV  <br/> |443  <br/> |_sip._tls.contoso.com  <br/> |sip.contoso.com  <br/> |Une interface externe pour votre service Edge d’accès. Cet enregistrement SRV est requis pour que les clients Skype Entreprise Server, Lync Server 2013 et Lync Server 2010 fonctionnent en externe. Vous en aurez besoin pour chaque domaine avec Skype Entreprise.  <br/> |
...
Skype Entreprise Server utilise des certificats pour sécuriser et chiffrer les communications entre les serveurs et de serveur à client. Comme vous vous y attendiez, vos certificats doivent avoir des enregistrements DNS pour vos serveurs qui correspondent à n’importe quel nom de sujet (SN) et autre nom de l’objet (SAN) sur vos certificats. Cela prendra du travail maintenant, lors de la phase de planification, pour vous assurer que vous avez les bons FQDN enregistrés dans DNS pour les entrées SN et SAN pour vos certificats.
...
Au minimum, le certificat affecté à vos interfaces de serveur Edge externes doit être fourni par une autorité de certification publique. Nous ne pouvons pas vous recommander une ca spécifique, mais nous avons une liste de ca, partenaires de certificats [de](/SkypeForBusiness/certification/services-ssl) communications unifiées, que vous pouvez consulter pour voir si votre ca préférée est répertoriée.
...
- Vous pouvez passer par l’installation de Skype Entreprise Server, puis le déploiement du serveur Edge. L’Assistant Déploiement de Skype Entreprise Server aura une étape pour générer une demande de certificat, que vous pouvez ensuite envoyer à votre cae choisie.
    
- Vous pouvez également utiliser Windows PowerShell commande pour générer cette demande, si celle-ci est plus en ligne avec les besoins de votre entreprise ou votre stratégie de déploiement.
...
- fabrikam .com : deuxième nom de domaine général (ajouté en tant que démonstration de ce qu’il faut demander si vous avez plusieurs domaines <span></span> SIP)
...
L’obtention d’une planification pour les ports et les pare-feu pour les déploiements de serveurs Edge Skype Entreprise Server peut vous éviter des jours ou des semaines de dépannage et de contrainte. Par conséquent, nous allons ré lister quelques tableaux qui indiqueront l’utilisation de notre protocole et les ports que vous devez avoir ouverts, entrants et sortants, à la fois pour les scénarios NAT et IP publics. Nous allons également avoir des tableaux distincts pour les scénarios d’équilibrage de la charge matérielle (HLB) et des instructions supplémentaires à ce niveau. Pour en savoir plus à partir de là, nous avons également des [scénarios](scenarios.md) de serveur Edge dans Skype Entreprise Server que vous pouvez consulter pour vos problèmes de déploiement particuliers.
...
|MTLS  <br/> |TCP  <br/> |50001  <br/> |N’importe lequel  <br/> |Interface interne du serveur Edge  <br/> |Contrôleur du service de journalisation centralisée utilisant Skype Entreprise Server Management Shell et les cmdlets du service de journalisation centralisée, les commandes de ligne de commande ClsController (ClsController.exe) ou agent (ClsAgent.exe) et la collection de journaux.  <br/> |
...
|:-----|:-----|:-----|:-----|:-----|:-----|
|XMPP  <br/> Non pris en charge dans Skype Entreprise Server 2019 |TCP  <br/> |5269  <br/> |N’importe lequel  <br/> |Service proxy XMPP (partage une adresse IP avec le service Edge d’accès)  <br/> |Le service proxy XMPP accepte le trafic provenant de contacts XMPP dans les fédérations XMPP définies.  <br/> |
|XMPP  <br/>Non pris en charge dans Skype Entreprise Server 2019 |TCP  <br/> |5269  <br/> |Service proxy XMPP (partage une adresse IP avec le service Edge d’accès)  <br/> |N’importe lequel  <br/> |Le service proxy XMPP envoie le trafic à partir de contacts XMPP dans les fédérations XMPP définies.  <br/> |
|Access/SIP(TLS)  <br/> |TCP  <br/> |443  <br/> |N’importe lequel  <br/> |**Adresse IP privée à l’aide de NAT :** Service Edge d’accès au serveur Edge <br/> **ADRESSE IP publique :** Adresse IP publique du service Edge d’accès au serveur Edge <br/> |Trafic SIP client à serveur pour l’accès des utilisateurs externes.  <br/> |
|Access/SIP(MTLS)  <br/> |TCP  <br/> |5061  <br/> |N’importe lequel  <br/> |**Adresse IP privée à l’aide de NAT :** Service Edge d’accès au serveur Edge <br/> **ADRESSE IP publique :** Adresse IP publique du service Edge d’accès au serveur Edge <br/> |Pour la connectivité de messagerie instantanée publique et fédérée à l’aide de SIP.  <br/> |
|Access/SIP(MTLS)  <br/> |TCP  <br/> |5061  <br/> |**Adresse IP privée à l’aide de NAT :** Service Edge d’accès au serveur Edge <br/> **ADRESSE IP publique :** Adresse IP publique du service Edge d’accès au serveur Edge <br/> |N’importe lequel  <br/> |Pour la connectivité de messagerie instantanée publique et fédérée à l’aide de SIP.  <br/> |
|Conférence web/PSOM(TLS)  <br/> |TCP  <br/> |443  <br/> |N’importe lequel  <br/> |**Adresse IP privée à l’aide de NAT :** Service Edge de conférence web de serveur Edge <br/> **ADRESSE IP publique :** Adresse IP publique du service Edge de conférence web du serveur Edge <br/> |Média de conférence web.  <br/> |
