...
---
# <a name="advanced-edge-server-dns-planning-for-skype-for-business-server-2015"></a>Côté serveur DNS une planification avancée pour Skype pour Business Server 2015
...
Lorsqu’il s’agit de Domain Name System (DNS) planification de Skype pour Business Server 2015, il existe un grand nombre de facteurs qui peuvent jouer dans votre décision. Si la structure de domaines de votre organisation est déjà en place, la question est peut-être de réviser la façon dont vous allez procéder. Commençons par les rubriques situées ci-dessous :
...
Skype pour les clients de l’entreprise sont similaires à des versions antérieures de clients Lync comment ils rechercher et accéder aux services Skype pour Business Server 2015. Cette section aborde en détail le processus d’emplacement du serveur.
...
5. sipinternal. \<domaine\>
     ...
     *Il s’agit d’un enregistrement d’hôte A pour le pool frontal ou directeur, peut être résolu que sur le réseau interne.* 
...
Cela présente certains défis. Le plus important est que la déconnexion évitée DNS est de la **non prise en charge** pour la mobilité. Ceci est dû aux enregistrements DNS LyncDiscover et LyncDiscoverInternal (LyncDiscover doit être défini sur votre serveur DNS externe, tandis que LyncDiscoverInternal doit être défini sur votre serveur DNS interne).
  
Nous en dresserons la liste les enregistrements DNS pour les zones internes et externes ici, mais vous trouverez des exemples détaillés dans la section exigences environnementales de serveur de transport Edge.
...
- Ce contoso.com interne contient :
  ...
  - Les enregistrements de DNS A et AAAA (si vous utilisez l’adressage IPv6) pour votre pool frontal, pool de directeur ou de directeur nom du pool et tous les serveurs internes exécutant Skype pour 2015 de serveur d’entreprise dans le réseau de votre organisation.
    
  - A et AAAA (si vous utilisez l’adressage IPv6) enregistrements DNS pour votre interface interne bord pour chaque Skype pour Business Server 2015 Edge Server dans votre réseau de périmètre.
    
  - Les enregistrements de DNS A et AAAA (si vous utilisez l’adressage IPv6) pour l’interface interne de chaque serveur proxy inversé dans le réseau de périmètre (qui est **facultatif** pour la gestion d’un proxy inverse).
    
  - DNS A et AAAA (si vous utilisez l’adressage IPv6) et les enregistrements SRV interne Skype pour la configuration automatique des clients Business Server 2015 (qui est **facultatif** ).
    
  - DNS A et AAAA (si vous utilisez l’adressage IPv6) ou les enregistrements CNAME pour la découverte automatique de Skype pour Business Server 2015 Web Services (qui est **facultatif** ).
    
- Tous vos Skype pour les interfaces de bord internes Business Server 2015 dans votre réseau de périmètre utilisent cette zone DNS interne pour résoudre des requêtes pour contoso.com.
    
- Tous les serveurs exécutant Skype pour Business Server 2015 et clients exécutant Skype pour 2015 de serveur d’entreprise dans le réseau d’entreprise, pointent vers les serveurs DNS internes pour la résolution des requêtes à contoso.com, ou utilisent le fichier hôte sur chaque serveur de transport Edge et de la liste A et le AAAA si ( vous utilisez l’adressage IPv6) enregistrements pour le serveur du tronçon suivant (spécialement pour le directeur ou directeur pool VIP, avant fin pool VIP ou serveur Standard Edition server).
...
- Ce contoso.com externe contient :
  ...
  - DNS A et AAAA (si vous utilisez l’adressage IPv6) ou CNAME, enregistrements, pour la découverte automatique de Skype pour les services web Business Server 2015. Ils s’utilisent avec la mobilité.
    
  - DNS A et AAAA (si vous utilisez l’adressage IPv6) et les enregistrements SRV pour l’interface externe du bord de chaque Skype pour Business Server 2015 Edge Server ou de charge équitablement VIP (HUMIDIFICATION à HLB) dans le réseau de périmètre.
    
  - Des enregistrements DNS SRV et AAAA (si vous utilisez l’adressage IPv6) et un pour l’interface externe du serveur proxy inverse ou (VIP pour un pool de serveurs proxy inversés), dans le réseau de périmètre.
    
  - DNS A et AAAA (si vous utilisez l’adressage IPv6) et les enregistrements SRV pour Skype pour la configuration automatique de client Business Server 2015 ( **facultatif** ).
    
## <a name="automatic-configuration-without-split-brain-dns"></a>Configuration automatique sans DNS split-brain
<a name="NoSplitBrainDNS"> </a>

Si vous n’utilisez pas DNS Déconnexion évitée, configuration automatique interne de clients exécutant Skype pour les entreprises ne fonctionne pas si vous utilisez l’une des solutions que nous avons ici. Pourquoi ? Étant donné que Skype pour Business Server 2015 nécessite l’URI SIP de l’utilisateur pour le correspond au domaine du pool frontal désigné pour la configuration automatique. Cela n’a pas changé depuis les versions précédentes de Lync Server.
...
- _sipinternaltls._tcp.contoso.com. 86400 IN SRV 0 0 5061 pool01.contoso.com
     ...
     *Si un utilisateur se connecte en tant que bob@contoso.com, cet enregistrement fonctionnerait pour la configuration automatique, comme le domaine SIP de l’utilisateur correspond au domaine du pool Front-End (contoso.com).* 
    
- _sipinternaltls._tcp.fabrikam.com. 86400 IN SRV 0 0 5061 pool01.fabrikam.com
    
     *Si un utilisateur se connecte en tant que alice@fabrikam.com, cet enregistrement fonctionnerait pour à nouveau la configuration automatique de la deuxième domaine, car le domaine SIP correspond le pool frontal pour ce domaine.* 
...
Donc, maintenant que nous savons tous que, si vous avez besoin impératif automatique pour votre Skype pour les clients d’entreprise sans déconnexion évitée DNS, vous avez ces options :
...
- **Zone interne correspondante**
    ...
    Par exemple, si vous avez un utilisateur hébergé sur pool01.contoso.net, mais des signes dans Skype pour professionnel en tant que bob@contoso.com, créer une zone DNS interne nommée contoso.com et qu’elle contient vous devez créer un et A DNS (AAAA si utilisée IPv6 addressing) enregistrement de pool01.contoso.com.
...
- **Repérage de la zone interne**
    ...
    Par exemple, si votre domaine SIP est contoso.com, et si vous avez un pool frontal appelé pool01 qui contient deux serveurs frontaux, vous devez les zones suivantes de haute précision et les enregistrements A dans votre DNS interne :
...
> [!NOTE]
> Vous verrez le pool frontal que FQDN apparaît deux fois, mais avec deux adresses IP différentes. C’est parce que l’équilibrage de charge DNS est utilisé. Si l’humidification à HLB est utilisée, il ne serait une entrée de pool frontal unique. 
  
> [!NOTE]
> En outre, les valeurs de nom de domaine complet du pool frontal changent entre les exemples contoso.com et fabrikam.com, mais les adresses IP restent les mêmes. C’est parce que les utilisateurs, ouvrez une session à partir d’un domaine SIP utiliseront le même pool de Front-End pour une configuration automatique. 
  
## <a name="dns-disaster-recovery"></a>Récupération d’urgence de DNS
<a name="DNSDR"> </a>

Pour configurer DNS pour rediriger Skype pour le trafic web de Business Server 2015 à votre restauration de reprise après sinistre (DR) et les sites de basculement, vous devez utiliser un fournisseur DNS qui prend en charge les GeoDNS. Vous pouvez configurer vos enregistrements DNS pour prendre en charge la récupération d’urgence, afin que les fonctions qui utilisent les services web se poursuivent même si un pool frontal entier tombe en panne. Cette fonctionnalité DR prend en charge les URL simples de découverte automatique, de réunion et de rendez-vous.
...
|**Enregistrement de GeoDNS**|**Enregistrements de pool**|**Enregistrements CNAME**|**Paramètres DNS (sélectionnez une option)**|
|:-----|:-----|:-----|:-----|
|Répondre à-int.geolb.contoso.com  <br/> |Pool1InternalWebFQDN.contoso.com  <br/> Pool2InternalWebFQDN.contoso.com  <br/> |Meet.contoso.com alias de Pool1InternalWebFQDN.contoso.com  <br/> Meet.contoso.com alias de Pool2InternalWebFQDN.contoso.com  <br/> |Round Robin entre pools  <br/> **SOIT** <br/> Utilisez le principal, se connecter au secondaire en cas de défaillance  <br/> |
|Répondre à-ext.geolb.contoso.com  <br/> |Pool1ExternalWebFQDN.contoso.com  <br/> Pool2ExternalWebFQDN.contoso.com  <br/> |Meet.contoso.com alias de Pool1ExternalWebFQDN.contoso.com  <br/> Meet.contoso.com alias de Pool2ExternalWebFQDN.contoso.com  <br/> |Round Robin entre pools  <br/> **SOIT** <br/> Utilisation du principal, connexion au secondaire en cas de défaillance  <br/> |
...
Par exemple, s’il y a trois serveurs frontaux dans un pool nommé pool01.contoso.com, les éléments suivants se produit :
  
- Clients exécutant Skype pour les entreprises de requête DNS pour pool01.contoso.com. La requête renvoie trois adresses IP et les met en cache comme suit (dans un ordre donné) :
...
- Si le client essaie de mise en cache de toutes les entrées sans établir une connexion, l’utilisateur reçoit une notification qu’aucune serveurs Skype pour Business Server 2015 ne sont disponibles pour le moment.
...
- Équilibrez la charge serveur-serveur SIP sur les serveurs Edge.
...
- Équilibrez la charge tout client-serveur le trafic entre les clients et les serveurs Edge.
    
Vous ne pouvez pas utiliser l’équilibrage de charge DNS pour :
  
- Trafic client-serveur web à vos serveurs frontaux ou un directeur.
    
Pour approfondir un peu plus sur le mode de sélection d’un enregistrement DNS SRV lorsque plusieurs enregistrements DNS sont retournées par une requête, le service Edge d’accès toujours sélectionne l’enregistrement avec la priorité numérique la plus basse et, si un départage est nécessaire, le poids numérique le plus élevé. Ceci est cohérent avec la [documentation de Internet Engineering Task Force](https://www.ietf.org/rfc/rfc2782.txt).
...
Alors, que se passe-t-il si plusieurs SRV DNS de même priorité et de même poids s’enregistrent comme renvoyés ? Dans ce cas Edge d’accès service choisit l’enregistrement SRV obtenu à partir du serveur DNS premier.
