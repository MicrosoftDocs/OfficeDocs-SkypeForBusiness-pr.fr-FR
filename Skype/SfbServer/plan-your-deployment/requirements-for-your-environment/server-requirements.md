...
|Disque   |SOIT :  <br/> • 4 disques durs ou plus de 1 0000 TPM avec au moins 72 Go d’espace disque libre (les disques doivent être dans une configuration RAID 1 2x).  <br/> OR  <br/> • Disques SSD (Solid State Drive) capables de fournir le même espace libre et des performances similaires à 4 disques mécaniques 10000 RPM.   |
...
Vous pouvez utiliser la mise en SQL ou SQL clustering avec Skype Entreprise Server 2015, elle est prise en charge. SQL La mise en miroir est définie par le biais Skype Entreprise Server de topologie. Si vous avez l’intention de SQL clustering de clustering, cette configuration est effectuée SQL Server.
