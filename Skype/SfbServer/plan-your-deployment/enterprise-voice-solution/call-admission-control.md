...
---
# <a name="plan-for-call-admission-control-in-skype-for-business-server-2015"></a>Planification du contrôle d’admission des appels dans Skype Entreprise Server 2015
 
Obtenir des informations sur l’appel de contrôle d’admission, qui peut empêcher des appels d’être appliqué si ils auraient qualité médiocre media dans Skype pour Business Server Voix Entreprise.
...
Les administrateurs définissent des stratégies CAC, qui sont appliquées par le Service de stratégie de bande passante qui est installé avec chaque pool frontal. Paramètres de CAC sont automatiquement propagées à tous les Skype pour Business Server serveurs frontaux dans votre réseau.
...
Appeler l’admission de contrôle n’est pas appliquée au-delà des limites de la Skype pour organisation de Business Server 2015. CAC ne peut pas être appliquée au trafic multimédia transitant par Internet, qui n’est pas géré par Skype pour Business Server 2015. CAC vérifications sont effectuées sur la partie de l’appel qui circule dans le réseau de l’entreprise si le point de terminaison appelé appartient à l’organisation et le serveur de transport Edge a été ajouté à la configuration du réseau, comme décrit dans le contrôle d’admission [appel déploiement : liste de vérification finale pour Skype pour Business Server 2015](../../deploy/deploy-enterprise-voice/final-checklist.md). Si le point de terminaison appelé n’appartient pas à l’organisation, comme dans le cas d’un utilisateur fédéré ou PIC, aucune vérification de stratégie de bande passante ne sera effectuée et l’appel sortant ignorera les restrictions du service Contrôle d’admission des appels.
  
### <a name="call-admission-control-of-pstn-connections"></a>Contrôle d’admission des appels des connexions RTC

Appel de contrôle d’admission est exécutoire sur le serveur de médiation que si elle est connectée à un PBX IP, une passerelle PSTN ou un SIP trunk. Étant donné que le serveur de médiation est un agent utilisateur DOS à DOS (B2BUA), il met fin à media. Il a des deux côtés de la connexion : un côté connecté à Skype pour Business Server et un côté passerelle, qui est connecté au SIP malles, IP/PBX ou passerelles RTPC. Pour plus d’informations sur les connexions RTPC, reportez-vous à la section [planifier la connectivité RTPC dans Skype pour Business Server 2015](pstn-connectivity-0.md).
  
CAC peut être appliqué sur les deux côtés du serveur de médiation, à moins que le contournement de média est activé. Si le contournement de média est activé, le trafic multimédia ne traversent pas le serveur de médiation, mais plutôt les flux directement entre le Skype pour client d’entreprise et de la passerelle. Dans ce cas, le contrôle d’admission des appels n’est pas nécessaire. Pour plus d’informations, reportez-vous à [planification des médias ignorer dans Skype pour entreprise 2015](media-bypass.md).
...
**Appeler l’application du contrôle d’admission sur connexions au RTC**
...
1. Identifier les concentrateurs/dorsales (appelées des régions de réseau) au sein de votre réseau d’entreprise.
    
2. Identifiez les bureaux ou les emplacements (appelés sites de réseau) dans chaque zone du réseau.
...
4. Déterminez les limites de bande passante pour chaque liaison réseau étendu (WAN).
    ...
    > [!NOTE]
    > Limites de bande passante voir combien de la bande passante sur une liaison réseau étendu qui est allouée à la Voix Entreprise et le trafic audio/vidéo. Lors d’une liaison WAN est décrite comme « contraints de la bande passante », la liaison WAN possède une limite de la bande passante est inférieure au trafic maximal attendu sur le lien. 
  
5. Identifiez les sous-réseaux IP affectés à chaque site réseau.
    
Pour expliquer ces concepts, nous allons utiliser l’exemple de topologie de réseau illustré dans la figure suivante.
  
**Exemple de topologie pour l’appel de contrôle d’admission**
...
Les composants de cet exemple de topologie sont décrits dans les sections suivantes. Pour plus d’informations sur comment cette topologie a été planifiée, y compris les limites de la bande passante, consultez [exemple : collecte des besoins pour un appel de contrôle d’admission dans Skype pour Business Server 2015](example-gathering-requirements.md).
...
Un site réseau représente un lieu physique pour votre organisation, par exemple, des bureaux, un ensemble de bâtiments ou un campus. Tout lieu physique qui comporte un réseau local (LAN) et une connectivité de réseau étendu (WAN) vers d’autres sites est considéré comme un site réseau. Commencez par l’inventaire de tous les bureaux de votre organisation. Dans notre exemple de topologie, la région réseau Amérique du Nord comporte les sites réseau suivants : New York, Chicago, Détroit, Portland, Reno et Albuquerque.
...
> [!NOTE]
> Toutes les valeurs de bande passante CAC représentent les limites de la bande passante maximale *unidirectionnel* .
  
> [!NOTE]
> Le Skype pour les fonctionnalités de stratégie de voix Business Server fournissent des vérifications de la possibilité de remplacer les règles de bande passante pour les appels entrants pour l’utilisateur (pas pour les appels sortants placés par l’utilisateur). Une fois la session établie, la consommation de bande passante est calculée avec précision. Ce paramètre doit être utilisé avec modération. Pour plus d’informations, consultez [créer ou modifier une stratégie de voix et configurer les enregistrements d’utilisation de TLS dans Skype pour entreprise 2015](../../deploy/deploy-enterprise-voice/voice-policy-and-pstn-usage-records.md) ou [Modifier une stratégie Voice et configurer les enregistrements d’utilisation RTPC](http://technet.microsoft.com/library/6c53aaf5-218b-4bd4-8cea-31bc9d53f1bd.aspx) dans la documentation de déploiement.
...
- Peer-to-peer des appels audio entre Skype pour les points de terminaison entreprise va utiliser RTAudio (8kHz) ou RTAudio (16kHz) lorsque vous prendre en compte la bande passante et l’ordre de priorité des codecs.
    
- Conférences téléphoniques entre Skype pour les points de terminaison Business et l’A / V Conferencing service utilisera G.722 ou sirène.
    
- Les appels au réseau téléphonique public commuté (RTPC) vers ou à partir de Skype pour les points de terminaison Business utilisera G.711, soit RTAudio (8kHz).
    
Utilisez le tableau suivant pour optimiser les paramètres de bande passante par session maximale.
  
**Utilisation de la bande passante par les codecs**

|**Codec**|**Spécification de la bande passante avec aucune correction aval des erreurs (FEC)**|**Spécification de la bande passante avec correction d’erreurs (FEC)**|
|:-----|:-----|:-----|
...
G.722, le codec par défaut pour Skype pour les conférences Business Server diffère complètement les codecs G.722.1 et Siren.
...
- Si un client de Communications Server 2007 ou de Communications Server 2007 R2 se connecte à un Skype pour le service de conférence Business Server (parce que ces clients ne prennent pas en charge le codec G.722).
    
**Utilisation de la bande passante par scénario**

|**Scénario**|**Spécification de la bande passante optimisée pour la quantité (Kbits/s)**|**Spécification de la bande passante pour le mode équilibrée (Kbits/s)**|**Exigences de bande passante optimisé pour la qualité (Kbits/s)**|
|:-----|:-----|:-----|:-----|
...
|Appels RTPC (entre Skype pour passerelle Business et RTPC, avec contournement de média)  <br/> |97 Kbits/s  <br/> |97 Kbits/s  <br/> |161 Kbits/s  <br/> |
|Appels RTPC entre (Skype pour les entreprises) et serveur de médiation, sans le contournement de média  <br/> |45 Kbits/s  <br/> |97 Kbits/s  <br/> |161 Kbits/s  <br/> |
|Appels RTPC (entre le serveur de médiation et la passerelle PSTN, sans le contournement de média)  <br/> |97 Kbits/s  <br/> |97 Kbits/s  <br/> |161 Kbits/s  <br/> |
|Appelle de Polycom Skype pour entreprise-  <br/> |101 Kbits/s  <br/> |101 Kbits/s  <br/> |101 Kbits/s  <br/> |
...
- Si vous souhaitez réacheminer des appels bloqués sur le réseau RTC, vérifiez les fonctionnalités et capacités correspondantes. Pour plus d’informations, consultez [Planification sortant du routage d’appels](http://technet.microsoft.com/library/37c55fa4-175a-4190-b9e4-c2e5ac7b9261.aspx).
