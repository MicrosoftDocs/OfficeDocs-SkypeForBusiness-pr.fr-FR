...
Résistance des communications vocales fait référence à la capacité des utilisateurs pour continuer l’émission et la réception des appels si un site central que hôtes Skype pour Business Server n’est pas disponible, que ce soit par une large zone réseau (étendu WAN) Échec ou cause d’une autre. Si un site central ne fonctionne pas, le service de voix en entreprise doit basculer vers un site de sauvegarde pour assurer la continuité du service. En cas de panne du réseau étendu, les appels de la succursale doivent être redirigés vers une passerelle RTC locale. Cette section traite de la planification pour la résistance des communications vocales en cas de panne du site central ou du réseau étendu.
  
## <a name="central-site-resiliency"></a>Résistance des sites centraux

De plus en plus d’entreprises ont plusieurs sites basés dans le monde entier. Accès aux services d’assistance et la possibilité de réaliser les tâches critiques lorsqu’un site central est hors service Gestion des services d’urgence, est essentielle pour toute solution de résistance de voix entreprise. Quand un site central devient indisponible, les conditions suivantes doivent être remplies :
  
- Le basculement vocal doit être fourni.
    
- Les utilisateurs qui enregistrent généralement avec le pool frontal sur le site central doivent être en mesure d’enregistrer avec un autre pool frontal. Pour ce faire, vous pouvez créer des enregistrements, chacun d'entre eux correspond à un pool directeur ou un pool frontal dans chacun de vos sites centraux plusieurs enregistrements SRV DNS. Vous pouvez régler la priorité et poids les enregistrements SRV afin que les utilisateurs qui sont pris en charge par ce site central obtiennent le pool directeur et Front-End correspondant à celles des autres enregistrements SRV.
...
Planification de la résilience vocale sur un site central nécessite une connaissance du rôle central joué par le Skype pour le serveur d’inscriptions de Business Server lors de l’activation du basculement vocal. Le Skype pour le serveur d’inscriptions de Business Server est un service qui permet l’authentification et l’inscription du client et fournit des services de routage. Il s’exécute sur tous les serveur Standard Edition server, serveur frontal, directeur ou Survivable Branch Appliance. Un pool de serveurs d’inscriptions se compose des Services de serveur d’inscriptions en cours d’exécution sur le pool frontal et qui résident sur le même site. Un Skype pour client Business découvre le pool frontal via le mécanisme de découverte suivantes : 
...
Une fois le Skype pour client d’entreprise se connecte au pool frontal, il est destiné à l’équilibrage de charge à un des serveurs frontaux du pool. Ce serveur frontal, redirige à son tour, le client à un serveur d’inscriptions préféré dans le pool.
  
Chaque utilisateur activé pour Enterprise Voice est affecté à un pool de serveurs d’inscriptions particulier, qui devient le pool de serveurs d’inscriptions principal de cet utilisateur. Sur un site donné, des centaines voire des milliers d’utilisateurs partagent généralement un seul pool de serveurs d’inscriptions principal. Pour calculer la consommation des ressources du site central par les utilisateurs de site de succursale qui recourent au site central pour accéder aux fonctionnalités de présence, de conférence ou de basculement, nous vous conseillons de considérer chaque utilisateur de site de succursale comme s’il était inscrit sur le site central. Il n’y a aucune limite sur le nombre d’utilisateurs de site de succursale, y compris les utilisateurs auprès d’un serveur Survivable Branch Appliance.
  
Pour garantir la résistance vocale en cas de défaillance du site central, le pool de serveurs d’inscriptions principal doit avoir un pool de serveurs d’inscriptions de sauvegarde associé, situé sur un autre site. La sauvegarde peut être configurée à l’aide de paramètres de résilience du Générateur de topologie. S’il existe une liaison réseau étendu résistante entre les deux sites, les utilisateurs dont le pool de serveurs d’inscriptions principal n’est plus disponible sont automatiquement dirigés vers le pool de serveurs d’inscriptions de sauvegarde.
  
Les étapes suivantes décrivent le processus de découverte et d’inscription des clients :
  
1. Un client découvre Skype pour Business Server via les enregistrements DNS SRV. Dans Skype pour Business Server, les enregistrements SRV DNS peuvent être configurés pour renvoyer plus d’un nom de domaine complet pour la requête DNS SRV. Par exemple, si l’entreprise Contoso possède trois sites centraux (Amérique du Nord, Europe et Asie-Pacifique) et un pool directeur sur chaque site central, les enregistrements DNS SRV peuvent pointer vers les noms de domaine complets du pool directeur sur chacun des trois sites. Dans la mesure où le pool directeur dans un des emplacements n’est disponible, le client peut se connecter au premier saut Skype pour Business Server.
    
    > [!NOTE]
    > À l’aide d’un pool directeur est facultatif. Un pool frontal peut être utilisé à la place. 
  
2. Le pool directeur informe le Skype pour client Business pool de serveurs d’inscriptions principal de l’utilisateur et le pool de serveurs d’inscriptions de sauvegarde.
    
3. Le Skype pour Business client tente de se connecter tout d’abord pool de serveurs d’inscriptions principal de l’utilisateur. Si le pool de serveurs d’inscriptions principal est disponible, le serveur d’inscriptions accepte l’inscription. Si le pool de serveurs d’inscriptions principal n’est pas disponible, le Skype pour Business client tente de se connecter au pool de serveurs d’inscriptions de sauvegarde. Si le pool de serveurs d’inscriptions de sauvegarde est disponible et a déterminé que pool de serveurs d’inscriptions principal de l’utilisateur n’est pas disponible (par détection d’un manque de pulsation pour un intervalle de basculement spécifié) le pool de serveurs d’inscriptions de sauvegarde accepte l’inscription de l’utilisateur. Quand le serveur d’inscriptions de sauvegarde détecte que le serveur d’inscriptions principal est de nouveau disponible, le pool de serveurs d’inscriptions de sauvegarde redirige les clients de basculement vers leur pool principal.
...
- La charge de chaque pool de serveurs d’inscriptions doit être équilibrée via l’équilibrage de la charge DNS, l’équilibrage de la charge matérielle ou les deux. Pour plus d’informations sur la planification de votre configuration de l’équilibrage de charge, voir [équilibrage de la configuration requise pour Skype pour les entreprises de charge](../../plan-your-deployment/network-requirements/load-balancing.md).
    
- Chaque utilisateur doit être affecté à un pool de serveurs d’inscriptions principal à l’aide de la soit Skype pour l’applet de commande **set-CsUser** Business Server Management Shell ou le Skype pour le panneau de configuration serveur Business.
...
- Si le site central contenu dans votre serveur d’administration principal et le site est susceptible d’être inactif pendant une longue période, vous devrez réinstaller vos outils de gestion au niveau du site de sauvegarde ; dans le cas contraire, vous ne pourrez pas modifier les paramètres de gestion.
    
### <a name="dependencies"></a>Dépendances

Skype pour Business Server dépend des composants logiciels et d’infrastructure suivants pour garantir la résistance vocale :
...
- Dépôt et récupération de messages vocaux 
  ...
  - Configuration de messagerie unifiée Exchange plan de numérotation de chaque utilisateur à inclure des serveurs de messagerie unifiée Exchange au niveau du site central et le site de sauvegarde, mais désigner les serveurs de messagerie unifiée Exchange sauvegarde désactivée. Si le site principal devient indisponible, l’administrateur Exchange doit marquer les serveurs de messagerie unifiée Exchange sur le site de sauvegarde comme étant activé.
    
    Si aucune des solutions précédentes n’est possible, la messagerie unifiée Exchange est alors disponible du site central devient indisponible.
...
Si vous souhaitez fournir une résilience de site de succursale, autrement dit, service d’Enterprise Voice de haute disponibilité, vous disposez de trois options pour ce faire :
  
- Survivable Branch Appliance
    
- Serveur SBS
    
- Un Skype complète pour le déploiement de serveur d’entreprise sur le site de succursale 
...
Si vous fournissez une résilience de site de succursale, en cas d’échec de la connexion WAN d’un site de succursale à un site central ou si le site central est inaccessible, les fonctionnalités vocales suivantes devraient continuer à être disponibles :
...
- Transfert d’appel, sonnerie simultanée de systèmes d’extrémité, la délégation d’appel et les services d’appel d’équipe, mais uniquement si la personne qui a délégué et délégué (par exemple, un responsable et administrateur responsable) ou tous les membres de l’équipe, sont configurés sur le même site
...
Les fonctionnalités suivantes seront disponibles uniquement si votre solution de résistance est un Skype à grande échelle pour le déploiement de serveur d’entreprise sur le site de succursale :
...
- Mise en service des nouveaux téléphones et clients, mais uniquement si les Services de domaine Active Directory est présent sur le site de succursale.
    
- Enhanced 9-1-1 (E9-1-1)
    
    Si E9-1-1 est déployé et la jonction SIP sur le site central n’est pas disponible car la liaison réseau étendu est arrêté, Survivable Branch Appliance achemine les appels E9-1-1 vers la passerelle de succursale local. Pour activer cette fonctionnalité, les stratégies de voix des utilisateurs du site de succursale doivent acheminer les appels vers la passerelle locale en cas de panne du WAN.
    
> [!NOTE]
> SBA (survivable branch office) n’est pas pris en charge pour XMPP. Les utilisateurs hébergés dans un SBA configurations ne sera pas en mesure d’envoyer des messages instantanés ou afficher la présence avec des contacts XMPP. 
  
### <a name="branch-site-resiliency-solutions"></a>Solutions de résistance de sites de succursale

Mettre en œuvre la résistance des sites de succursale pour votre organisation présente des avantages évidents. Plus précisément, si vous perdez la connexion vers le site central, les utilisateurs de site de succursale continuera d’Enterprise Voice service et la messagerie vocale (si vous configurez les paramètres de réacheminement des messages vocaux). Cependant, pour les sites comportant moins de 25 utilisateurs, une solution de résistance peut ne pas être assez rentable. 
...
Survivable Branch Appliance inclut également des jonctions PSTN, des ports analogiques et une carte réseau Ethernet. 
  
Si la connexion WAN du site de succursale à un site central devient indisponible, les utilisateurs de succursale interne continuent à être enregistrés avec le serveur d’inscriptions de Survivable Branch Appliance et obtenir service ininterrompu vocal à l’aide de la connexion Survivable Branch Appliance vers le RTC. De même, les utilisateurs du site de succursale se connectant de leur domicile ou d’autres emplacements distants pourront s’enregistrer avec un serveur d’inscriptions du site central si la liaison de réseau étendu vers le site de succursale est indisponible. Ces utilisateurs disposeront de la fonctionnalité de communication unifiée complète, la seule exception étant que les appels entrants vers le site de succursale seront transmis à la messagerie vocale. Lorsque la connexion de réseau étendu redevient disponible, les utilisateurs du site de succursale disposent de nouveau des fonctionnalités complètes de communication. Le basculement vers le serveur Survivable Branch Appliance ni la restauration du service nécessite la présence d’un administrateur informatique.
  
Skype pour Business Server prend en charge jusqu'à deux Survivable Branch Appliance sur un site de succursale. 
...
Pour plus d’informations sur ces étapes, consultez [déploiement d’un serveur Survivable Branch Appliance ou un serveur](http://technet.microsoft.com/library/cb780c14-dc5f-41ba-8092-f20ae905bd16.aspx) dans la documentation de déploiement.
  
|**Phase**|**Étapes**|**Droits d’utilisateur**|
|:-----|:-----|:-----|
|Configurer les Services de domaine Active Directory pour le Survivable Branch Appliance  <br/> |**Sur le site central :** <br/>  Créer un compte d’utilisateur de domaine (ou l’identité d’entreprise) pour le technicien qui installera et activera le Survivable Branch Appliance sur le site de succursale. <br/>  Créez un compte d’ordinateur (avec le nom de domaine complet applicable (FQDN)) pour les Survivable Branch Appliance dans les Services de domaine Active Directory. <br/>  Dans le Générateur de topologie, créer et publier le Survivable Branch Appliance. <br/> |Le compte d’utilisateur du technicien doit être membre du groupe RTCUniversalSBATechnicians. Survivable Branch Appliance doit appartenir au groupe RTCSBAUniversalServices, ce qui se produit lorsque vous utilisez le Générateur de topologie.  <br/> |
|Installer et activer le Survivable Branch Appliance.  <br/> |**Sur le site de succursale :** <br/>  Connectez le Survivable Branch Appliance à un port Ethernet et un port PSTN. <br/>  Démarrez le Survivable Branch Appliance. <br/>  Rejoindre le Survivable Branch Appliance pour le domaine, en utilisant le compte d’utilisateur de domaine créé pour le Survivable Branch Appliance sur le site central. Définissez le nom de domaine complet et l’adresse IP de sorte qu’ils correspondent au nom de domaine complet créé pour le compte d’utilisateur. <br/>  Configurer le Survivable Branch Appliance à l’aide de son interface utilisateur. <br/>  Testez la connectivité PSTN. <br/> |Le compte d’utilisateur du technicien doit être membre du groupe RTCUniversalSBATechnicians.  <br/> |
   
#### <a name="survivable-branch-server-details"></a>Serveur Survivable Branch Server en détail

Dans le Générateur de topologie créer le site de succursale, ajoutez le serveur Survivable Branch Server à ce site et exécutez la Skype pour l’Assistant de déploiement Business Server sur l’ordinateur où vous souhaitez installer le rôle.
...
Préparer les utilisateurs pour la résistance des sites de succursale en définissant leur pool de serveurs d’inscriptions comme le Survivable Branch Appliance (SBA) ou un serveur Survivable Branch Server.
  
#### <a name="registrar-assignments-for-branch-users"></a>Affectations de serveurs d’inscriptions aux utilisateurs de succursale

Quelle que soit la solution de résistance de site de succursale choisie, vous devez affecter un serveur d’inscriptions principal à chaque utilisateur. Les utilisateurs de site de succursale doivent toujours enregistrer avec le serveur d’inscriptions sur le site de succursale, quel que soit si celui-ci se trouve dans le Survivable Branch Appliance, Survivable Branch Server ou autonome Skype pour Business Server Standard ou Enterprise Edition serveur. Un enregistrement de ressource de service (SRV) DNS (Domain Name System) est requis pour qu’un client puisse détecter automatiquement son pool de serveurs d’inscriptions. Si le serveur Survivable Branch Appliance devient indisponible, il s’agit de comment les clients de site de succursale détectera automatiquement le serveur d’inscriptions de sauvegarde.
  
Si un site de succursale ne dispose pas d’un serveur DNS, il existe deux alternatives pour configurer la découverte du Survivable Branch Appliance ou serveur Survivable Branch Server :
  
- Configurer l’option DHCP 120 sur le serveur de Configuration protocole DHCP (Dynamic Host) du site de succursale pour pointer vers le nom de domaine complet (FQDN) du Survivable Branch Appliance ou serveur Survivable Branch Server.
    
- Configurer le Survivable Branch Appliance ou un serveur Survivable Branch Server pour répondre aux requêtes DHCP 120.
    
#### <a name="voice-routing-for-branch-users"></a>Routage des communications vocales des utilisateurs de succursale

Nous vous recommandons de créer une stratégie VoIP (Voice over Internet Protocol) distincte au niveau utilisateur pour les utilisateurs d’un site de succursale. Cette stratégie doit inclure un itinéraire principal qui utilise la passerelle de serveur Survivable Branch Appliance ou une filiale et un ou plusieurs itinéraires de sauvegarde qui utilisent une jonction avec une passerelle de réseau téléphonique commuté public sur le site central. Si l’itinéraire principal n’est pas disponible, l’itinéraire alternatif faisant appel à une ou plusieurs passerelles de site central est utilisé à la place. De cette manière, quel que soit l’où un utilisateur est inscrit, sur le site de succursale du serveur d’inscriptions ou le pool de serveurs d’inscriptions de sauvegarde sur le site central : aucune stratégie VoIP de l’utilisateur est toujours en vigueur. Il est primordial de tenir compte de ce point pour les scénarios de basculement. Par exemple, si vous devez renommer le Survivable Branch Appliance ou reconfigurer le Survivable Branch Appliance pour se connecter à une pool de serveurs d’inscriptions sur le site central de sauvegarde, vous devez déplacer les utilisateurs de site de succursale vers le site central pour la durée. (Pour plus d’informations sur la modification du nom ou reconfigurer un Survivable Branch Appliance, voir [annexe b : gestion d’un serveur Survivable Branch Appliance](http://technet.microsoft.com/library/2ec9d505-6d39-491c-9524-8cf36866b855.aspx) dans la documentation de déploiement). Si les utilisateurs n’ont pas les stratégies au niveau utilisateur VoIP ou les plans de numérotation au niveau de l’utilisateur, lorsque les utilisateurs sont déplacés vers un autre site, les stratégies de VoIP d’au niveau du site et la numérotation au niveau du site du site central des plans s’appliquent aux utilisateurs par défaut, au lieu du site de succursale au niveau du site VoIP plans de numérotation et les stratégies. Dans ce scénario, leurs appels échoueront, à moins que les stratégies VoIP et les plans de numérotation au niveau du site utilisés par le pool de serveurs d’inscriptions de sauvegarde puissent également s’appliquer aux utilisateurs du site de succursale. Par exemple, si les utilisateurs d’un site de succursale basé au Japon sont déplacés sur un site central situé à Redmond, il est probable qu’un plan de numérotation dont les règles de normalisation ajoutent le préfixe +1425 à tous les appels à 7 chiffres ne traduiront pas correctement les appels de ces utilisateurs.
...
Vous devez également faire face aux scénarios de numéros de poste spécifiques où la liaison de réseau étendu entre un site de succursale et un site central n’est pas disponible et où un appel émanant d’un site de succursale doit être acheminé sur le réseau téléphonique commuté. Pendant une panne de réseau étendue, si un utilisateur de site de succursale appelle un utilisateur du site central uniquement par numérotation extension de l’utilisateur site central, vous devez disposer une règle de traduction sortante qui ajoute le numéro de téléphone complet de l’utilisateur site central. Si l’URI de ligne d’un utilisateur contient le numéro de téléphone complet de votre organisation et le numéro de poste unique de l’utilisateur au lieu d’un numéro de téléphone complet qui est unique à l’utilisateur, vous devez avoir une règle de traduction sortante qui ajoute à la place de numéro de téléphone complet de votre organisation . Par exemple :
  
|**Description**|**Modèle de correspondance**|**Conversion**|**Exemple**|
|:-----|:-----|:-----|:-----|
|Traduit les numéros à 5 chiffres pour le numéro de téléphone et l’extension d’un utilisateur  <br/> |^(\d{5})$  <br/> |+14255550123;poste=$1  <br/> |10001 devient +14255550123;ext=10001  <br/> |
|Traduit les numéros à 5 chiffres en numéro de téléphone de votre organisation et une extension d’un utilisateur  <br/> |^(\d{5})$  <br/> |+14255550100;poste=$1  <br/> |10001 devient +14255550100;ext=10001  <br/> |
...
Numéros configurés pour des utilisateurs individuels à sélection directe ou non une liaison WAN est disponible, si votre organisation n’a pas l’URI de ligne pour un utilisateur contient le numéro de téléphone de votre organisation et le numéro de poste unique de l’utilisateur, puis vous devez configurer votre téléphone numéro URI de ligne organisation avec un numéro qui est accessible par l’homologue de jonction ou la passerelle PSTN au niveau du site de succursale. Vous devez également configurer la ligne numéro de téléphone de votre organisation URI à inclure son propre extension unique pour les appels routés vers ce numéro.
  
#### <a name="preparing-for-voice-mail-survivability"></a>Préparation de la survivabilité de la messagerie vocale

Messagerie unifiée Exchange (MU) est généralement installé uniquement sur un site central et pas au niveau des sites de succursale. Un appelant doit pouvoir laisser un message vocal, même si la liaison de réseau étendu entre un site de succursale et le site central n’est pas disponible. Par conséquent, la configuration de l’URI de ligne pour le numéro de téléphone du standard automatique Exchange UM qui fournit la messagerie vocale pour les utilisateurs de site de succursale nécessite les considérations particulières, en plus de la stratégie de voix, de numérotation de plan et les règles de normalisation applicables à la messagerie vocale nombre.
  
Survivable Branch Appliances (SBA) et les serveurs Survivable Branch Server fournissent la survivabilité de la messagerie vocale pour les utilisateurs de succursale pendant une panne de réseau étendue. Plus précisément, si vous utilisez un serveur Survivable Branch Appliance ou serveur Survivable Branch Server et le réseau étendu n’est pas disponible, le SBA ou serveur Survivable Branch Server redirige les appels sans réponse via la passerelle PSTN vers la messagerie unifiée Exchange sur le site central. Avec un SBA ou un serveur Survivable Branch Server, les utilisateurs peuvent également récupérer des messages vocaux via le réseau RTC pendant une panne de réseau étendue. Enfin, pendant une panne de réseau étendue le Survivable Branch Appliance ou serveur Survivable Branch Server des files d’attente les notifications d’appels manqués et les télécharge vers le serveur de messagerie unifiée Exchange lorsque le réseau étendu est rétablie. Pour garantir que le réacheminement des messages vocaux est robuste, veillez à ajouter une entrée de nom de domaine complet du pool site central et une entrée pour le FQDN du serveur Edge pour le fichier hosts sur le serveur Survivable Branch Server. À défaut, il est possible que la résolution DNS arrive à expiration si vous ne disposez pas de serveur DNS sur le site de succursale.
...
- Un administrateur de Microsoft Exchange doit configurer Exchange UM automatique standard pour accepter uniquement les messages. Cette configuration désactive toutes les autres fonctions génériques, comme le transfert à un utilisateur ou un opérateur, et limite le rôle du standard automatique à la seule réception des messages. Sinon, l’administrateur Exchange peut utiliser un standard automatique générique ou personnalisé pour router l’appel vers un opérateur.
    
- Le Skype pour administrateur Business Server doit prendre le numéro de téléphone du standard automatique et utiliser ce numéro de téléphone comme numéro **exchange um standard automatique** dans les paramètres pour le Survivable Branch Appliance ou le serveur de succursale de réacheminement des messages vocaux.
    
- Le Skype pour l’administrateur du serveur d’entreprise doit obtenir l’abonné de messagerie unifiée Exchange numéro d’accès et utiliser comme le numéro **d’accès abonné** dans les paramètres pour le Survivable Branch Appliance ou serveur Survivable Branch Server de réacheminement des messages vocaux .
    
- Le Skype pour administrateur Business Server doit configurer la messagerie unifiée Exchange afin que ce seulement un plan de numérotation soit associé à tous les utilisateurs de succursale qui ont besoin d’accéder à la messagerie vocale pendant une panne de réseau étendue.
    
- Lors de la liaison WAN n’est pas disponible, les appels aux utilisateurs de site de succursale peuvent être acheminés vers la messagerie vocale de la messagerie unifiée Exchange (MU) de l’utilisateur, mais uniquement si la stratégie de voix est appliqué à l’appel spécifie un numéro de téléphone de la messagerie vocale qui est unique et n’inclut pas un numéro de poste nombre.
...
Configuration requise matérielle et logicielle est intégré dans le Survivable Branch Appliance. Cependant, il est également recommandé que chaque site de succursale déploie un serveur DHCP afin d’obtenir des adresses IP du client ; sinon, lorsque le bail DHCP expire, les clients ne disposent plus de connectivité IP.
  
Si les serveurs DNS d’entreprise se trouvent uniquement dans les sites centraux, les utilisateurs de site de succursale ne pourront pas se connecter aux pendant une panne de réseau étendue, et par conséquent Skype pour la découverte Business Server qui utilise le DNS SRV (enregistrement de ressource de service (SRV)) échoue. Pour garantir un réacheminement rapide lors d’une panne de réseau étendu, les enregistrements DNS doivent être mis en cache au niveau du site de succursale. Si le routeur de succursale prend en charge le cache DNS, activez-le à cette fin. Vous pouvez également déployer un serveur DNS au niveau de la succursale. Il peut s’agir d’un serveur autonome ou une version du Survivable Branch Appliance qui prend en charge les fonctionnalités de DNS. Pour plus d’informations, contactez votre fournisseur Survivable Branch Appliance.
  
> [!NOTE]
> Il n’est pas nécessaire de disposer d’un contrôleur de domaine sur un site de succursale. Survivable Branch Appliance authentifie les clients à l’aide d’un certificat spécial qu’il envoie le client en réponse à la demande de certificat du client lorsqu’il se connecte. 
  
Skype pour les clients d’entreprise peut découvrir le Skype pour Business Server à l’aide de DHCP, Option 120 (Option de serveur d’inscriptions SIP). La configuration peut prendre l’une des deux formes suivantes :
  
- Configurer le serveur DHCP sur le site de succursale pour répondre aux requêtes DHCP 120, qui renvoient le nom de domaine complet du serveur d’inscriptions sur les Survivable Branch Appliance ou serveur Survivable Branch Server.
    
- Activer Skype pour Business serveur DHCP. Lorsque cette option est activée, la Skype pour le serveur d’inscriptions de Business Server répond aux requêtes DHCP, Option 120. Notez que le serveur d’inscriptions ne répond pas aux requêtes DHCP autres que DHCP, option 120.
...
Enfin, les utilisateurs de site de succursale doivent être configurés pour Enterprise Voice et mis en service avec un point de terminaison des communications unifiées approprié.
...
Pour plus d’informations, voir [configuration du serveur pour Skype pour Business Server 2015](../../plan-your-deployment/requirements-for-your-environment/server-requirements.md) dans la documentation de planification.
