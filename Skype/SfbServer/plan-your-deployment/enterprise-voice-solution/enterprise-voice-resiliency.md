...
- Dépôt et récupération de messages vocaux
  ...
  - Configurez le plan de numérotation de messagerie un Exchange de chaque utilisateur pour qu’il inclue des serveurs de messagerie un Exchange à la fois sur le site central et sur le site de sauvegarde, mais désignez les serveurs de messagerie un Exchange de sauvegarde comme désactivés. Si le site principal devient indisponible, l’administrateur Exchange doit marquer les serveurs de messagerie un Exchange sur le site de sauvegarde comme activés.
    ...
    Si aucune des solutions précédentes n’est possible, la Exchange de la sécurité ne sera pas disponible en cas d’indisponibilité du site central.
...
Skype Entreprise clients peuvent découvrir les Skype Entreprise Server à l’aide de l’option DHCP 120 (option de bureau d’enregistrement SIP). La configuration peut prendre l’une des deux formes suivantes :
...
- Activer Skype Entreprise Server DHCP. Lorsque cette option est désactivée, le Skype Entreprise Server de bureau d’enregistrement répond aux requêtes d’option DHCP 120. Notez que le serveur d’inscriptions ne répond pas aux requêtes DHCP autres que DHCP, option 120.
