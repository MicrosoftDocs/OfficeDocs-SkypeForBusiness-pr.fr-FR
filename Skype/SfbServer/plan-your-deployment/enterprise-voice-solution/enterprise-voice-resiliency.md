...
- Dépôt et récupération de messages vocaux
  ...
  - Configurez le plan de numérotation de messagerie unie Exchange de chaque utilisateur pour qu’il inclue les serveurs de messagerie unie Exchange à la fois sur le site central et sur le site de sauvegarde, mais désignez les serveurs de messagerie un jour Exchange de sauvegarde comme désactivés. Si le site principal devient indisponible, l’administrateur Exchange doit marquer les serveurs de messagerie un jour exchange sur le site de sauvegarde comme étant activés.
...
Nous vous recommandons également de créer un plan de numérotation et une stratégie de voix au niveau utilisateur, et de les affecter aux utilisateurs de site de succursale. Pour plus d’informations, voir [Create or modify a dial plan in Skype for Business Server](../../deploy/deploy-enterprise-voice/dial-plans.md) and Create the [VoIP Routing Policy for Branch Users in](/previous-versions/office/lync-server-2013/lync-server-2013-create-the-voip-routing-policy-for-branch-users) the Deployment documentation.
...
- L’administrateur Skype Entreprise Server doit configurer la messagerie unie Exchange de sorte qu’un seul plan de numérotation soit associé à tous les utilisateurs de succursale qui ont besoin d’accéder à la messagerie vocale pendant une panne du réseau wan.
...
- Configurez le serveur DHCP sur le site de succursale pour répondre aux requêtes DHCP 120, qui retournent le nom de domaine (FQDN) du serveur d’inscriptions sur le Survivable Branch Appliance ou le serveur Survivable Branch Server.
