...
Découvrez les serveurs de médiation dans Skype Entreprise Server, notamment ses topologies pris en charge et ses relations avec les branches M:N, le contournement de média et le contrôle d’admission des appels.
  
Pour déployer Voix Entreprise, vous devez déployer un ou plusieurs serveurs de médiation. 
  
Le serveur de médiation traduit la signalisation entre votre infrastructure Voix Entreprise interne et une passerelle PSTN (réseau téléphonique commuté) ou une session SIP (Session Initiation Protocol). Dans certains déploiements, il traduit également le média lui-même entre ces points.
...
- Chiffrement et déchiffrement de SRTP côté Skype Entreprise Server. 
...
La figure suivante illustre les protocoles de signalisation et multimédias utilisés par le serveur de médiation lors de la communication avec une passerelle PSTN de base et l’infrastructure Voix Entreprise de données.
...
- **Passerelle PSTN.** Une passerelle qualifiée Skype Entreprise Server doit implémenter l’équilibrage de charge DNS, ce qui permet à une passerelle de réseau téléphonique commuté (PSTN) qualifiée d’agir en tant qu’équilibreur de charge pour un pool de serveurs de médiation, et par conséquent d’équilibrer la charge des appels dans le pool.
...
Pour plus d’informations sur le contournement de média ou le contrôle d’admission des appels, voir [Plan for media bypass in Skype for Business](media-bypass.md) or Plan for call admission control in Skype for Business [Server](call-admission-control.md).
...
Par défaut, le serveur de médiation Skype Entreprise est coqueté avec le serveur Standard Edition Server, un pool frontal ou le Survivable Branch Appliance. Tous les serveurs de médiation d’un pool frontal doivent être configurés de manière identique.
...
- Effectuer un équilibrage de charge DNS (Domain Name System) de couche réseau entre les serveurs de médiation dans un pool (ou router le trafic de manière uniforme vers tous les serveurs de médiation d’un pool)
