...
**Résumé :** Lisez cette rubrique pour en savoir plus sur les nouvelles fonctionnalités Skype Entreprise Server 2015. Pour plus d’informations sur la nouvelle expérience client, consultez [la Skype Entreprise Lync : découvrez les nouveautés](https://go.microsoft.com/fwlink/p/?LinkId=529022).
...
- Poursuivre ou rejoindre des conversations
...
SQL Server instances de cluster de failover tirent parti de la fonctionnalité WSFC (Server Failover Clustering) Windows pour fournir une haute disponibilité locale via la redondance au niveau de l’instance de serveur ( instance de cluster de failover). Un FCI est une instance unique de SQL Server qui est installée sur les serveurs de clustering de Windows (WSFC) et, éventuellement, sur plusieurs sous-réseaux.
...
Skype Entreprise Server 2015 active les fonctionnalités d’administration et de gestion de déploiement hybride en introduisant les fonctionnalités suivantes :
