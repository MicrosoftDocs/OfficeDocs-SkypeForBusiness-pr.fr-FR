...
description: 'Résumé : Affecter des stratégies de code confidentiel par utilisateur aux utilisateurs dans Skype Entreprise Server.'
...
La stratégie de numéro d’identification personnelle (PIN) de conférence rendez-vous est l’un des paramètres individuels d’un compte d’utilisateur qui peuvent être configurés dans le Skype Entreprise Server Panneau de configuration.
...
7. Dans **Attribuer des stratégies**, sous **Stratégie de code confidentiel**, effectuez l’une des opérations suivantes :
    ...
    > [!NOTE]
    > Étant donné que vous pouvez configurer plusieurs stratégies à l’aide de la boîte de dialogue **Attribuer des** stratégies, **\<Keep as is\>** elle est sélectionnée par défaut pour chaque stratégie de la boîte de dialogue. Continuez à utiliser la stratégie précédemment attribuée à l’utilisateur sans apporter de modification au paramètre.

   - Autorisez Skype Entreprise Server à choisir automatiquement la stratégie de niveau global ou, si elle est définie, la stratégie au niveau du site.
...
## <a name="assigning-a-per-user-pin-policy-by-using-windows-powershell-cmdlets"></a>Affectation d’une stratégie de code confidentiel Per-User à l’aide d’applets de commande Windows PowerShell

Vous pouvez affecter des stratégies de code confidentiel par utilisateur à l’aide de Windows PowerShell et de l’applet **de commande Grant-CsPinPolicy**. Vous pouvez exécuter cette applet de commande à partir de Skype Entreprise Server Management Shell ou d’une session distante de Windows PowerShell. Pour plus d’informations sur l’utilisation des Windows PowerShell distantes pour se connecter à Skype Entreprise Server, consultez [Administration De Microsoft Lync Remote PowerShell ».](https://blog.insideo365.com/2011/08/remote-lync-powershell-administration/) Le processus est le même dans Skype Entreprise Server.
...
- La commande suivante affecte la stratégie de code confidentiel par utilisateur RedmondUsersPinPolicy à tous les utilisateurs travaillant dans la ville de Redmond. Pour plus d’informations sur le paramètre LdapFilter utilisé dans cette commande, consultez [Get-CsUser](/powershell/module/skype/get-csuser).
...
Pour plus d’informations, consultez [Grant-CsPinPolicy](/powershell/module/skype/grant-cspinpolicy).
