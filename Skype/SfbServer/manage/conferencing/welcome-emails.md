...
---
# <a name="send-welcome-email-to-dial-in-users-in-skype-for-business-server-2015"></a>Envoi d’un message électronique de bienvenue aux utilisateurs rendez-vous dans Skype Entreprise Server 2015
 
**Résumé :** Découvrez comment la bienvenue aux utilisateurs de conférence à distance dans Skype pour Business Server 2015.
  
Après avoir configuré les conférences à distance et test pour vérifier qu’il fonctionne correctement, vous devez définir initiale identification personnel (PIN) pour les utilisateurs et avertir les utilisateurs de la disponibilité de la fonctionnalité. Vous pouvez inclure des instructions introduction tels que le code PIN initial et le lien vers la page web de paramètres pour une conférence à distance. 
  
En général, vous utilisez l’applet de commande **Set-CsClientPin** pour réinitialiser les PIN, mais vous pouvez utiliser la procédure de cette rubrique si vous souhaitez envoyer un e-mail de bienvenue initial avec les informations de code PIN. Si vous ne souhaitez pas envoyer ce message, utilisez plutôt **Set-CsClientPin**.
...
**SmtpServer** Par défaut, le script utilise la valeur de l’environnement réservées variable **$PSEmailServer** pour ce paramètre. Si la variable **$PSEmailServer** n’est pas définie, vous devez spécifier ce paramètre.
