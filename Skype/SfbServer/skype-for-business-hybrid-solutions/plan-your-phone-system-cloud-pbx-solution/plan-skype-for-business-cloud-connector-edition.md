...
description: Recherchez des informations sur Skype Entreprise, version Cloud Connector, un ensemble de machines virtuelles (VM) qui implémentent la connectivité PSTN sur site avec le système téléphonique (PBX cloud).
ms.openlocfilehash: 75abeba6c97b59b4795fc3399ff6a983db57bbff
ms.sourcegitcommit: 03ff569a0b7a8e04d7b0ab32f370a9a537fa7fe7
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 04/28/2021
ms.locfileid: "52064710"
---
...
Cloud Connector Edition peut être la solution appropriée pour votre organisation si vous n'avez pas encore de déploiement Lync Server ou Skype Entreprise Server existant. Si vous recherchez toujours la solution de système téléphonique adaptée à votre entreprise, consultez [les solutions de téléphonie Microsoft.](/microsoftteams/cloud-voice-landing-page)
...
- Cloud Connector ne nécessite pas un déploiement complet de Skype Entreprise Server sur site.
    ...
    Actuellement, Cloud Connector ne peut pas co-exister avec des serveurs locaux Lync ou Skype Entreprise. Si vous souhaitez déplacer des utilisateurs Lync ou Skype Entreprise existants vers Microsoft 365 et continuer à fournir des services téléphoniques locaux à vos utilisateurs, envisagez un système téléphonique avec connectivité sur site à l'aide d'un déploiement Skype Entreprise Server existant. Pour plus d'informations, voir [Plan your Phone System (Cloud PBX) solution](/microsoftteams/cloud-voice-landing-page.md) and Plan Phone System with [on-premises PSTN connectivity in Skype for Business Server](plan-phone-system-with-on-premises-pstn-connectivity.md).
...
- Si vous souhaitez fournir des conférences à des utilisateurs hébergés sur Cloud Connector, vous pouvez acheter une licence de conférence PSTN ou payer à mesure que vous recevez une offre d'audioconférence de Microsoft.
...
- Cloud Connector 2.1 et les ultérieures prend en charge la surveillance de Cloud Connector à l'aide d'Operations Management Suite (OMS). Pour plus d'informations, voir [Surveiller Cloud Connector à l'aide d'Operations Management Suite (OMS)](monitor-cloud-connector-using-operations-management-suite-oms.md)
...
- Si vous avez Cloud Connector version 2.0 ou ultérieure et que vous déployez la configuration maximale de 16 appliances (avec un matériel plus important), votre site PSTN peut gérer jusqu'à 8 000 appels simultanés. Si vous déployez la version la plus petite, la limite prise en charge est de 800.
  ...
  - Si vous avez une version antérieure et que vous déployez la configuration maximale de 3 + 1 (avec un matériel plus important), votre site PSTN peut gérer jusqu'à 1 500 appels simultanés. Si vous déployez la version la plus petite, la limite prise en charge est de 150.
...
Le diagramme suivant montre une appliance Cloud Connector edition unique au sein d'un seul site PSTN. Notez que Cloud Connector se compose de quatre machines vm installées sur un ordinateur hôte physique qui se trouve dans un réseau de périmètre à des fins de sécurité.
...
- Si un serveur proxy est requis sur l'ordinateur hôte pour naviguer sur Internet, vous devez apporter les modifications de configuration suivantes :
  ...
  - **Registre :** HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\Internet Settings] ProxySettingsPerUser dword:000000000

  - **Stratégie de groupe :** Composants \> Windows De modèles d'administration d'ordinateurs Internet Explorer : définir des paramètres de proxy par \> ordinateur \> (et non par utilisateur)
...
|Interface externe edge cloud connector  <br/> |N’importe lequel  <br/> |N’importe lequel  <br/> |TCP(MTLS) 5061  <br/> |
|Interface externe edge cloud connector  <br/> |N’importe lequel  <br/> |N’importe lequel  <br/> |TCP 80  <br/> |
|Interface externe edge de Cloud Connector  <br/> |N’importe lequel  <br/> |N’importe lequel  <br/> |UDP 53  <br/> |
|Interface externe edge de Cloud Connector  <br/> |N’importe lequel  <br/> |N’importe lequel  <br/> |TCP 53  <br/> |
|Interface externe edge de Cloud Connector  <br/> |N’importe lequel  <br/> |TCP 50 000-59 999  <br/> |N’importe lequel  <br/> |
...
|Nom du contrôleur de domaine Cloud Connector  <br/> |Nom du contrôleur de domaine.  <br/> Nom dans le fichier .ini : « ServerName »  <br/> |Doit comprendre au moins 15 caractères. Entrez le nom Netbios uniquement.  <br/> |
...
|ADRESSE IP externe du edge du relais multimédia (si Edge se trouve derrière NAT)  <br/> |Si votre edge se trouve derrière nat, vous devez également spécifier l’adresse publique de l’appareil NAT.  <br/> Nom dans le fichier .ini : « ExternalMRPublicIPs » sous « Parameters for a pool of Edge Servers »  <br/> ||
...
|Mot de passe de l’administrateur en mode sans échec  <br/> Version 1.4.2 uniquement  <br/> |Mot de passe administrateur en mode sans échec pour le domaine CC interne.  <br/> ||
