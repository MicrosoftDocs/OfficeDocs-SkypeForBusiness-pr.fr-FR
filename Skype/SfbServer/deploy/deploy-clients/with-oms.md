...
> [!IMPORTANT]
> Bien que la configuration minimale, la Suite de gestion des opérations permet de surveiller un ordinateur exécutant un système d’exploitation Windows, il y a encore quelques étapes Skype salle systèmes spécifiques que vous devez prendre avant de commencer le déploiement des agents sur tous les systèmes de salle de Skype périphériques.
...
### <a name="configure-operations-management-suite-to-collect-skype-room-systems-event-logs"></a>Configurer la Suite de gestion des opérations pour collecter des journaux des événements de systèmes de salle de Skype
...
Vous devez configurer la Suite de gestion des opérations pour collecter les journaux requis pour surveiller l’état de périphériques et d’applications de systèmes de salle Skype. Les appareils de v2 de systèmes de salle Skype utilisent le journal des événements Skype salle systèmes.

Pour configurer la Suite de gestion des opérations pour collecter les événements de systèmes de salle Skype, voir [les sources de données de journal des événements Windows dans le journal Analytique](https://docs.microsoft.com/azure/log-analytics/log-analytics-data-sources-windows-events)
...
Vous devez préparer la Suite de gestion des opérations pour être en mesure de surveiller les événements relatifs aux systèmes de salle Skype. Vous devez déployer les agents Operations Management Suite à une ou deux périphériques de systèmes de salle Skype que vous avez accès physique à et ces derniers sont les périphériques de test générer des données et l’envoyer à l’espace de travail journal Analytique.
...
2.  Répertorie les événements générés par un périphérique de systèmes de salle de Skype :
...
4.  Générer un problème de configuration matérielle et valider que les événements requis sont enregistrés dans la Suite de gestion des opérations.
    1.  Déconnectez un des périphériques sur le système de systèmes de salle Skype de test. Cela peut être la caméra, téléphone mains libres, microphone ou affichage de la salle de premier plan
...
5.  Générer un problème d’application et valider que les événements requis sont enregistrés.
    1.  Modifier la configuration de l’application Skype salle systèmes, puis tapez une paire adresse/mot de passe de protocole SIP (Session Initiation) incorrect.
    2.  Attendez 10 minutes pour le journal des événements doivent être renseignés dans la Suite de gestion des opérations.
...
| **Champ JSON**               | **Champ personnalisé OMS**       | **ID d’événement** |
|------------------------------|----------------------------|-----------------|
...
1.  Téléchargez le [tableau de bord](http://download.microsoft.com/download/9/0/D/90D4826A-9FD2-47D2-B911-97BF1737F4F7/SkypeRoomSystems_v2.omsview).
...
3.  Nom de la mosaïque **Skype salle systèmes**.
...
4.  Définissez les propriétés de **Mosaïque** :<br>
    ...
    **Mosaïque de requête :**```Event | where EventLog == "Skype Room System" and SRSOperationName_CF == "Heartbeat" and TimeGenerated > ago(20m) | summarize AggregatedValue = count() by Computer | count```
...
3.  Définissez les propriétés de **Mosaïque** :<br>
    ...
    **Mosaïque de requête :**```Event | where EventLog == "Skype Room System" and SRSOperationName_CF == "Heartbeat" | summarize LastHB = max(TimeGenerated) by Computer | where LastHB < ago(20m) | count```
...
1.  Sélectionnez le **nombre et la liste** dans la galerie, puis ajoutez une nouvelle mosaïque.
2.  Définir les propriétés **générales** :<br>
    **Titre de groupe :** Configuration matérielle<br>
    **Nouveau groupe :** Sélectionné
3.  Définissez les propriétés de **Mosaïque** :<br>
    **Légende :** Périphériques qui a rencontré une erreur matérielle dans la dernière heure <br>
    **Mosaïque de requête :**```Event | where EventLog == "Skype Room System" and EventLevelName == "Error" and EventID == "3001" and TimeGenerated > ago(1h) | summarize AggregatedValue = count() by Computer | count```
4.  Définissez les propriétés de **liste** :<br>
...
### <a name="create-a-tile-that-displays-skype-room-systems-application-versions"></a>Créer une mosaïque qui affiche les versions de l’application systèmes de salle de Skype
...
3.  Définir les propriétés de **Mosaïque** .<br>
    ...
    **Mosaïque de requête :**```Event | where EventLog == "Skype Room System" and EventLevelName == "Error" and EventID == "2001" and TimeGenerated > ago(1h) | summarize AggregatedValue = count() by Computer | count```
...
3.  Définir les propriétés de **Mosaïque** .<br>
    ...
    **Mosaïque de requête :**```Event | where EventLog == "Skype Room System" and EventID == "4000" and TimeGenerated > ago(24h) | summarize AggregatedValue = count() by Computer | count```
4.  Définir les propriétés de la **liste** .<br>
...
<a name="Alerts"></a> Périphérique lorsqu’un Skype salle systèmes rencontre un problème, Microsoft Operations Management Suite peut générer des alertes pour avertir les administrateurs avec les détails du problème.
...
> [!NOTE]
> Les exemples suivants envoient des alertes par courrier électronique lorsqu’un périphérique Skype salle systèmes génère un matériel ou une erreur d’application. 
...
Configurer une règle d’alerte qui vérifie pour les périphériques de systèmes de salle Skype qui ont des problèmes liés au matériel au sein de la dernière heure.
1.  Connectez-vous au [portail Microsoft Operations Management Suite](http://aka.ms/omsportal).
...
5.  Configurer les paramètres d’alerte en utilisant les informations ci-dessous :<br>
    **Nom de règle :** Alerte d’échec de salle Skype systèmes matériel<br>
    ...
    **Objet de l’e-mail :** Alerte d’échec de salle Skype systèmes matériel<br>
    **Destinataires :** Inclure les adresses de messagerie, des points-virgules<br>

6.  Cliquez sur **Enregistrer**.

### <a name="configure-an-email-alert-for-skype-room-systems-application-issues"></a>Configurer un message d’alerte pour les problèmes d’application systèmes de salle de Skype

Configurer une règle d’alerte qui vérifie pour les périphériques de systèmes de salle Skype qui ont des problèmes des applications au sein de la dernière heure.
...
4.  Configurer les paramètres d’alerte en utilisant les informations ci-dessous :<br>
    **Nom de règle :** Alerte d’échec de salle Skype systèmes Application<br>
    ...
    **Fréquence de l’alerte :** 1 heure<br>
    **Nombre de résultats :** Supérieur à 0<br>
    **Objet de l’e-mail :** Alerte d’échec de salle Skype systèmes Application<br>
...
<a name="configure_all_devices"></a> Après les tableaux de bord et les alertes sont configurés, vous pouvez installer et configurer agents Suite de gestion des opérations sur tous les périphériques de systèmes de salle Skype pour effectuer votre déploiement de surveillance.
...
Si vous déjà déployé vos périphériques Skype salle systèmes avant d’implémenter des opérations de gestion Suite, vous pouvez utiliser le script fourni pour installer et configurer les agents à l’aide de stratégies de groupe Active Directory.
...
4.  Créer un nouvel objet de stratégie de groupe et l’affecter à l’unité d’organisation où se trouvent les comptes d’ordinateur Skype salle systèmes.
...
7.  Périphériques de systèmes de salle Skype doivent installer et configurer l’agent Microsoft Monitoring du deuxième redémarrage.
...
[Gérer les systèmes de salle Skype v2 périphériques avec OMS](../../manage/skype-room-systems-v2/oms.md)