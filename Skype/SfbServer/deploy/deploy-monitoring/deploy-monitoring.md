...
Une autre amélioration importante de la surveillance dans Skype Entreprise Server est le fait que Skype Entreprise Server Monitoring Reports prend désormais en charge IPv6 : les rapports qui utilisent le champ Adresse IP affichent les adresses IPv4 ou IPv6 en fonction de : 1) le SQL  requête utilisée ; et, 2) où l’adresse IPv6 est stockée ou non dans la base de données de surveillance.
...
Cette documentation vous indique le processus d’installation et de configuration des rapports de surveillance et de surveillance Skype Entreprise Server. La documentation donne des instructions détaillées qui vous aideront à effectuer les opérations suivantes :
