...
Bien que les agents de collecte de données unifié sont automatiquement installés et activés sur chaque serveur frontal, cela ne signifie pas que vous commencera automatiquement à collecter des données d’analyse au moment où que vous avez terminé l’installation Skype pour Business Server. Au lieu de cela, vous devez effectuer deux opérations : vous devez associer vos pools de fin/avant les serveurs frontaux avec une base de données de surveillance, et vous devez activer l’enregistrement détail des appels (CDR) et/ou de qualité de l’expérience (QoE) monitoring à l’étendue globale et/ou l’étendue du site.
...
3. Dans le volet **paramètre de modifier l’enregistrement (détails des appels)** , sélectionnez **Activer la surveillance des détails** , puis sur **Valider**.
...
Si vous préférez, vous pouvez également activer la surveillance de QoE à partir de dans la Skype pour le panneau de configuration serveur Business. À partir du Panneau de configuration, procédez comme suit :

1. Cliquez sur **surveillance**.

2. Sous l’onglet **Données de qualité de l’expérience** , double-cliquez sur le paramètre **Global** .

3. Dans le volet **paramètre de qualité de l’expérience (QoE)** , sélectionnez **Activer la surveillance des données QoE** , puis sur **Valider**.

Comme indiqué précédemment, les exemples précédents activer la surveillance dans l’étendue globale ; Autrement dit, ils permettent des détails des appels et QoE monitoring au sein de votre organisation. Sinon, vous pouvez créer des paramètres de configuration CDR et QoE distincts au niveau du site et puis sélectivement activer ou désactiver la surveillance pour chaque site. Par exemple, vous pouvez activer la surveillance de votre site de Redmond, encore désactiver CDR pour votre site de Dublin de surveillance des détails des appels. Pour plus d’informations sur la gestion de vos paramètres de configuration de surveillance, voir la rubrique [Configure appeler l’enregistrement des détails et les paramètres de qualité de l’expérience dans Skype pour Business Server](call-detail-recording-and-qoe.md)du guide de déploiement.
