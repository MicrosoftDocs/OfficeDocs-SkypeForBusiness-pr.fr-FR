...
Créer ou modifier un groupe d’agents dans Skype pour Business Server Voix Entreprise du groupe réponse.
...
Un agent qui doit signer dans et hors du groupe, qui est différent de la signature ou non Skype pour les entreprises, est appelé un agent officiel. Les agents formels doivent être connectés au groupe pour recevoir des appels routés vers le groupe. Cela peut s’avérer utile pour les agents qui répondent à temps partiel aux appels du groupe. Agents formels signer et leurs groupes, en cliquant sur un élément de menu dans Skype pour les entreprises à ouvrir le navigateur Internet de Windows Internet Explorer et afficher une page Web console.
  
Un agent qui ne signe pas dans ou hors du groupe est appelé un agent informel. Agents informels sont automatiquement connectées au groupe lorsqu’ils à Skype pour les entreprises, et ils ne peuvent pas de connexion du groupe.
...
### <a name="to-use-skype-for-business-server-control-panel-to-create-or-modify-an-agent-group"></a>Utiliser Skype pour le panneau de configuration de Business Server pour créer ou modifier un groupe d’agents
...
7. Dans **Stratégie de participation**, sélectionnez l’une des options ci-dessous pour définir le comportement de connexion du groupe :
   ...
   - Sélectionnez **Informel** pour spécifier que les agents du groupe n’ont pas besoin de se connecter au groupe ou de s’en déconnecter. Agents sont automatiquement connectées au groupe lors de leur inscription à Skype pour les entreprises.
    
   - Sélectionnez **Formel** pour spécifier que les agents du groupe doivent se connecter au groupe et s’en déconnecter. Lorsque vous sélectionnez cette option, les agents, cliquez sur un élément de menu dans Skype pour les entreprises à ouvrir Internet Explorer et afficher une console de page Web pour la signature et le groupe.
...
9. Dans **Méthode de routage**, sélectionnez la méthode de transmission des appels aux agents dans le groupe, comme suit :
   ...
   - Proposer un nouvel appel de tout d’abord à l’agent qui a été la plus longue inactivité (a eu une présence **disponible** ou **inactif** dans Skype pour entreprise la plus longue), cliquez sur **la plus longue d’inactivité**. 
   ...
   - Pour proposer un nouvel appel à tous les agents qui sont signés dans Skype pour les entreprises et l’application de groupe de réponse en même temps, indépendamment de leur présence en cours, cliquez sur **surveillance**. Les utilisateurs configurés comme agents peuvent voir tous les appels en attente et y répondre, dans l’ordre qui leur convient. L’appel est envoyé au premier agent qui l’accepte, et les autres agents ne peuvent alors plus voir l’appel.
...
10. Dans **Agents**, spécifiez la méthode de création de votre liste d’agents :
   ...
   - Pour utiliser une liste de distribution Microsoft Exchange Server comme groupe d’agents, cliquez sur **Utiliser une liste de distribution de courrier électronique existante**, puis tapez l’adresse de messagerie de la liste de distribution (par exemple, NetworkSupport@contoso.com) dans **Adresse de la liste de distribution**.
    ...
    > [!IMPORTANT]
    > Si vous utilisez une liste de distribution de courrier électronique, des membres masqués ou listes masquées risquent d’être visibles à l’administrateur de groupe de réponse ou les utilisateurs. 
  
    Les appartenances au groupe d’agents masquées ou les listes masquées peuvent devenir visibles, comme suit :
    
     - Si une liste de distribution a été configurée de sorte que l’appartenance est masqué et l’administrateur de groupe de réponse affecte la liste de distribution à la liste de l’agent, les utilisateurs peuvent appeler le groupe pour savoir quels sont les membres. 
    
     - Si une liste de distribution a été configurée de sorte qu’il soit masqué dans la liste d’adresses globale Exchange, l’administrateur de groupe de réponse peut être en mesure de voir la distribution de liste et l’assigner à la liste des agents si le processus de réponse groupe possède des droits d’utilisateur appropriés et autorisations, même si l’administrateur n’a pas les autorisations et droits d’utilisateur appropriés.
...
3. **Nouveau-CsRgsAgentGroup** permet de créer un nouveau groupe d’agent. **Set-CsRgsAgentGroup** permet de modifier un groupe existant de l’agent. À partir de la ligne de commande, exécutez la commande suivante :
...
[Nouvelle-CsRgsAgentGroup](https://docs.microsoft.com/powershell/module/skype/new-csrgsagentgroup?view=skype-ps)
  
[Ensemble-CsRgsAgentGroup](https://docs.microsoft.com/powershell/module/skype/set-csrgsagentgroup?view=skype-ps)
