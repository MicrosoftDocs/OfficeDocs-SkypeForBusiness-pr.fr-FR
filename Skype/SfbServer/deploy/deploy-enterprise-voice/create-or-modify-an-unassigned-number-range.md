...
Utilisez une des procédures suivantes pour configurer des plages de numéros non attribués pour l’application de l’annonce.
  
> [!IMPORTANT]
> Avant de configurer la table des numéros non attribuée, votre système doit déjà soit les annonces définis ou définir un Standard automatique de messagerie unifiée Exchange (MU). 
...
### <a name="to-use-skype-for-business-server-control-panel-to-configure-unassigned-phone-numbers"></a>Utiliser Skype pour le panneau de configuration de Business Server pour configurer des numéros de téléphone non affecté
...
7. Si, dans l’étape précédente, vous avez cliqué sur **Annonce**, procédez comme suit :
    ...
    a. Sous le **nom de domaine complet du serveur de destination**, cliquez sur **Sélectionner**, cliquez sur l’ID de service du service d’Application qui exécute l’application de l’annonce qui va traiter les appels entrants sur cette plage de numéros non attribués, puis cliquez sur **OK**.
...
10. Dans la page **Numéro non attribué**, vérifiez que les plages de numéros non attribués s’affichent dans l’ordre voulu. Pour déplacer une plage dans la table, cliquez sur un ou plusieurs noms consécutifs dans la liste de plages, puis cliquez sur la flèche vers le haut ou vers le bas.
    ...
    > [!TIP]
    > Skype pour Business Server recherche dans la table numérique non affectée de haut en bas et utilise la première plage qui correspond au numéro non affecté. Si des plages se chevauchent et qu’une plage spécifie une action de dernier recours, vérifiez qu’elle se trouve en bas de la liste. 
  
11. Une fois que vous disposez des plages de numéros non attribués dans l’ordre souhaité, cliquez sur **Valider tout**.
    
### <a name="to-use-skype-for-business-server-management-shell-to-configure-unassigned-phone-numbers"></a>Utiliser Skype pour Business Server Management Shell pour configurer des numéros de téléphone non affecté
...
3. **Nouveau-CsUnassignedNumber** permet de créer une nouvelle plage de numéros non attribuée. **Set-CsUnassignedNumber** permet de modifier une plage de numéros non attribuée existante.
...
### <a name="to-use-skype-for-business-server-control-panel-to-delete-an-unassigned-number-range"></a>Utiliser Skype pour Business Server du Panneau de configuration pour supprimer une plage de numéros non attribuée
...
3. Dans la ligne de commande, tapez :
   ...
   ```
    ...
    > [!NOTE]
    > Pour plus d’informations sur plus d’options, reportez-vous à la section [Remove-CsCallParkOrbit](https://docs.microsoft.com/powershell/module/skype/remove-cscallparkorbit?view=skype-ps). 
...
[Ensemble-CsUnassignedNumber](https://docs.microsoft.com/powershell/module/skype/set-csunassignednumber?view=skype-ps)
