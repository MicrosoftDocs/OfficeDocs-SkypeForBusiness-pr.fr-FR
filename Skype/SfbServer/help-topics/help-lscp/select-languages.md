---
title: Sélection des langues
ms.author: kenwith
author: kenwith
manager: serdars
ms.date: 3/23/2015
ms.audience: ITPro
ms.topic: article
f1_keywords:
- ms.lync.lscp.ConfCAASelLang
ms.prod: skype-for-business-itpro
localization_priority: Normal
ms.assetid: cb9f98e3-6123-4e5a-a360-8ee1be6977f1
description: Sélectionnez les langues que vous souhaitez utiliser pour votre numéro d’accès à la conférence rendez-vous. Pour plus d’informations, voir créer ou modifier un numéro d’accès à distance en conférence.
ms.openlocfilehash: 0ff21e6f1553d881066df98a03b0ec5c66f350a4
ms.sourcegitcommit: 7d819bc9eb63bfd85f5dada09f1b8e5354c56f6b
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 03/28/2018
---
# <a name="select-languages"></a>Sélection des langues
 
Sélectionnez les langues que vous souhaitez utiliser pour votre numéro d’accès à la conférence rendez-vous. Pour plus d’informations, voir [créer ou modifier un numéro d’accès à distance en conférence](http://technet.microsoft.com/library/06f55c28-57f8-4d4e-8313-9740846796d9.aspx).
  

