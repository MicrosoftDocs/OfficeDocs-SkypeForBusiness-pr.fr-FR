...
description: Dans le Générateur de topologies, vous pouvez modifier les paramètres de port utilisés pour les deux vos services web internes et externes. En outre, et si vous déployez l’équilibrage de la charge système de nom de domaine (DNS), vous pouvez utiliser le Générateur de topologies pour configurer le nom de domaine pleinement qualifié (FQDN) du pool qui résout les adresses IP physiques de tous les serveurs dans ce pool.
...
3. Si vous modifiez les propriétés d’un pool Enterprise Edition, vous avez la possibilité de sélectionner **Remplacer le nom de domaine complet**. Ne sélectionnez cette option que si vous utilisez l’équilibrage de la charge DNS. Si vous utilisez l’équilibrage de la charge DNS, sélectionnez **Remplacer le nom de domaine complet**, puis, dans la zone de texte, tapez le nom de domaine complet du pool résolu en adresses IP physiques de tous les serveurs de ce pool. Si vous n’utilisez pas l’équilibrage de la charge DNS et si vous n’avez pas sélectionné **Remplacer le nom de domaine complet**, vous ne pouvez pas modifier le nom de domaine complet des services web internes. Les services web interne FQDN est l’URL utilisée par les utilisateurs internes à se connecter sur Skype pour Business Server.
