...
> [!NOTE]
> Vous pouvez utiliser le panneau de configuration ou les applets de commande de Management Shell pour activer le contrôle d’admission des appels. 
...
- **Activer le contrôle d’admission des appels** Sélectionnez ce paramètre pour activer le CAC. Effacez ce paramètre pour désactiver le CAC pour l’ensemble de votre réseau. Pour activer le CAC, vous devez avoir configuré votre réseau pour le CAC. Pour plus d’informations, reportez-vous à la section [déploiement du contrôle d’admission des appels dans Skype entreprise Server](../../../deploy/deploy-enterprise-voice/deploy-call-admission-control.md) dans la documentation de déploiement.
    
- **Pool frontal pour exécuter le contrôle d’admission des appels** Si vous avez activé le CAC, vous pouvez modifier le pool qui s’exécute. Sélectionnez la liste déroulante dans la liste déroulante.
