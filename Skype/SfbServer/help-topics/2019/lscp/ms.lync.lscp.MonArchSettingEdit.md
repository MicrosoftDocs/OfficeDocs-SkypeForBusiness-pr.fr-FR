...
> [!NOTE]
> Les configurations d’archivage s’appliquent aux utilisateurs de Skype Entreprise Server et, si vous activez l’option d’intégration Microsoft Exchange pour utiliser Exchange pour stocker les données d’archivage dans Microsoft Exchange, aux utilisateurs d’exchange. Toutefois, certaines options sont implémentées légèrement différemment pour les utilisateurs d’Exchange, comme décrit dans la section suivante. 
...
- **Nom** Chaque configuration d’archivage nécessite un nom. Le nom est déterminé par le type de configuration que vous ajoutez ou modifiez :
  ...
  - **Configuration du pool** Spécifiez un nom approprié, qui peut être le nom d’un pool frontal ou d’un serveur Standard Edition Server spécifique dans votre déploiement. Par exemple, Division Marketing.
...
- **Intégration de Microsoft Exchange** Sélectionnez cette option si vous avez des utilisateurs qui sont homed sur Exchange. Avec cette option, Exchange est utilisé pour stocker les données de ces utilisateurs, si leurs boîtes aux lettres ont été placées en In-Place. Si tous vos utilisateurs sont do domicile sur Exchange, vous n’avez pas besoin de configurer des bases de données SQL Server pour le stockage des données d’archivage.
