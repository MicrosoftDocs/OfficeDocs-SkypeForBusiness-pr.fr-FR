...
> [!NOTE]
> Les configurations d’archivage s’appliquent aux utilisateurs sur Skype Entreprise Server et, si vous activez l’option d’intégration Microsoft Exchange pour utiliser Exchange pour stocker les données d’archivage dans Microsoft Exchange, aux utilisateurs qui sont Exchange. Toutefois, certaines options sont implémentées légèrement différemment pour les utilisateurs Exchange, comme décrit dans la section suivante. 
