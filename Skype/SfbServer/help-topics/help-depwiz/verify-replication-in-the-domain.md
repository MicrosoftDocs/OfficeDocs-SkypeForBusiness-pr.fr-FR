...
Pour vérifier la réplication de la préparation du domaine dans **étape 1 : préparer le schéma**, il est nécessaire d’exécuter une applet de commande à partir de la Skype pour Business Server Management Shell Lync Server Management Shell. Pour exécuter l’applet de commande Windows PowerShell, ouvrez une session sur un ordinateur qui est membre du domaine que vous avez préparé et en tant que membre du groupe Admins du domaine. Procédez comme suit :
...
2. Dans Windows PowerShell, tapez ce qui suit :
  ...
  ```
    ...
    Si vous omettez le paramètre Domain, le domaine local est utilisé. Cette applet de commande renvoie une valeur de **LC_DOMAIN_SETTINGS_STATE_READY** si la préparation du domaine a réussi.
