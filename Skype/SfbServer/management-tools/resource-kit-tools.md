...
**ExécutezOCSResKit.msi** pour une installation simple. Le .msi installe tous les outils dans le chemin d’accès suivant : **%Program Files%\Skype Entreprise Server 2015\ResKit**. Ce dossier contient des outils exécutables autonomes. Les outils qui ont également des fichiers de prise en charge se font dans leurs propres sous-dossiers.
...
ABSConfig peut être démarré en tapant **ABSConfig.exeà une** invite de commandes. L’interface utilisateur de l’outil ABSConfig est présentée ci-dessous.
...
> [!NOTE]
>  Si l’outil Bandwidth Policy Service Monitor subit une défaillance lorsqu’il se connecte à l’un des services de stratégie de bande  passante configurés, les informations des onglets Informations sur l’ordinateur et Info topologie ne sont pas remplies. Toutefois, il est possible que l’outil se connecte initialement, mais par la suite perd sa connexion au service. Dans ce cas, les administrateurs peuvent voir des informations obsolètes. Il **existe un** horodat de dernière mise à jour sur chacun des onglets qui permet aux administrateurs de voir quand les données ont été mises à jour pour la dernière fois pour un service de stratégie de bande passante particulier.
...
L’utilisation des WanLinkLogCollector.exe se fait en trois étapes :
...
- **Emplacement des fichiers journaux du serveur** Emplacement du dossier où sont stockés les journaux du serveur de stratégie de bande passante. Il s’agit généralement \<fileserver\>\\<choix de FE\>\AppServerFiles\PDP.
...
- **-r \<n\>** : répertorie les \<n\> derniers appels par parcés. Les informations affichées sont les suivantes :
...
Appels par parcés récemment : le paramètre -r \<n\> répertorie les \<n\> derniers appels par parcés, comme illustré

![Appels récemment par parcés dans le parcomètre d’appel.](../media/Reskit_2012_Tools_Documentation_Image21.jpg)

Réservation d’orbite de test : le paramètre -t \<n\> teste la réservation d’une orbite dans la base de données, comme illustré
 ...
 **Les** composants logiciels Skype Entreprise Server 2015 doivent être installés avant d’être en cours d’exécution de DBAnalyze.
...
- Afficher les données de configuration de topologie réseau CAC.
...
![Téléchargement de la configuration réseau.](../media/Reskit_2012_Tools_Documentation_Image23.jpg)
 ...
 Chargez et affichez la topologie du réseau cac à partir d’un fichier journal de serveur de stratégie de bande passante dans un **format** graphique : les serveurs de stratégie de bande passante Skype Entreprise Server 2015 enregistrent la topologie réseau CAC dans le cadre du mécanisme de journalisation sous l’emplacement de partage de fichiers Skype Entreprise Server 2015. Skype Entreprise Server 2015 peuvent afficher un tel fichier dans un format graphique à l’aide de l’option Configuration d’Open **Network**, comme illustré ci-dessous.
...
![Enregistrement de la configuration réseau sous forme d’image.](../media/Reskit_2012_Tools_Documentation_Image26.jpg)
 ...
 Afficher les données de <strong>configuration</strong> de la topologie réseau cac :Skype Entreprise Server 2015 les administrateurs peuvent afficher les données de configuration réseau associées telles que les régions réseau, les sites réseau, les profils de bande passante et les adresses IP de sous-réseau de site dans un format textuel à l’aide de l’option Afficher les données de configuration réseau, comme illustré ci-dessous.
...
L’outil De migration des annonces de numéros non signés permet à un administrateur Skype Entreprise Server 2015 de déplacer la configuration des numéros non signés pris en service par l’application d’annonce d’une Skype Entreprise Server source ou d’un pool vers une Skype Entreprise Server ou Pool.
...
- **Migration des paramètres de configuration** vers une nouvelle version de Skype Entreprise Server : Contoso est en cours de migration vers Skype Entreprise Server 2015 et, dans le cadre du processus de migration, le Skype Entreprise Server  l’administrateur souhaite déplacer la configuration des numéros non affectations serviceée par l’application d’annonce du déploiement Lync Server 2013 vers le nouveau déploiement Skype Entreprise Server 2015. Pour déplacer les paramètres de configuration, l’administrateur Skype Entreprise Server utilise l’outil De migration des annonces de numéro non signé.

- Roll **back a deployment from Skype Entreprise Server 2015 to Lync Server 2013:** Due unexpected factors, Contoso has to roll back the migration to the new Skype Entreprise Server 2015 deployment. Pour minimiser les perturbations du service, l’administrateur Skype Entreprise Server utilise l’outil De migration des annonces de numéro non affectation pour revenir à la configuration du déploiement Skype Entreprise Server 2015 vers le déploiement Lync Server 2013.
