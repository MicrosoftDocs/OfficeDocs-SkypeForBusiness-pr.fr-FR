...
---
# <a name="using-the-skype-for-business-server-2015-stress-and-performance-tool"></a>Utilisation de l’outil Stress and Performance de Skype Entreprise Server 2015
 
Pour exécuter l’outil Stress and Performance de Skype Entreprise Server 2015, vous devez être en mesure de gérer les utilisateurs, les contacts et les profils utilisateur, de configurer l’outil pour l’exécuter, puis de passer en revue les résultats produits par l’outil.
  
Quatre domaines sont impliqués dans l’exécution de l’outil Stress and Performance de Skype Entreprise Server 2015 (l’exécutable est LyncPerfTool.exe) :
...
Vous devez utiliser l’outil de mise en service utilisateur (UserProvisioningTool.exe) de Skype Entreprise Server 2015 (SB 2015) pour créer des utilisateurs et des contacts pour vos tests de contrainte et de performances.
...
-  Numéros de téléphone des États-Unis : numéros de téléphone attribués à l’utilisateur en plus de l’URI SIP utilisé pour le routage des appels entrants et sortants dans la recherche de numéros inverses ( RNL).
...
Vous devez utiliser l’outil de mise en service des utilisateurs de Skype Entreprise Server pour créer des utilisateurs et des contacts pour la simulation de chargement.
  
**L’outil de mise en service** des utilisateurs de Skype Entreprise Server est installé avec le package d’outils Stress and Performance de Skype Entreprise **Server.** Assurez-vous que le programme d’installation du package (CapacityPlanningTool.msi) a été exécuté sur le serveur frontal ou le serveur Standard Edition Server que vous avez l’intention de tester.
  
Vous pouvez démarrer l’outil de mise en service des utilisateurs de Skype Entreprise Server en exécutant le UserProvisioningTool.exe de fichier (situé dans %InstalledDirectory%LyncStressAndPerfTool\LyncStress) sur le serveur frontal ou sur le serveur Standard Edition.
...
1. Dans le champ **FQDN** du pool frontal, tapez le nom de domaine complet (FQDN) du serveur Standard Edition Ou le pool frontal dans lequel vous souhaitez héberger les utilisateurs.
...
6. Dans le **champ Code de zone téléphonique,** tapez le code de zone à trois chiffres à utiliser dans tous les comptes d’utilisateur test. Assurez-vous que le code de zone que vous avez choisi n’entre pas en conflit avec les indicatifs régionaux des autres utilisateurs dans AD.
...
9. Dans le champ **Index** de démarrage, indiquez le numéro de départ qui sera utilisé comme suffixe pour le préfixe du nom d’utilisateur (par exemple, le préfixe est « TestUser » et le prénom se terminera par « 0 » dans l’exemple ci-dessous.)
...
> [!NOTE]
> Seuls les numéros de téléphone au format américain sont pris en charge. Les numéros de téléphone sont toujours attribués aux utilisateurs, et tous les utilisateurs créés par UserProvisioningTool.exe sont activés pour Voix Entreprise par défaut. Tous les scénarios qui utilisent le numéro de téléphone, tels que les Standard automatique de conférence ou les appels UC-PSTN, utilisent ce numéro de téléphone pour router correctement les appels. Pour cette raison, *chaque utilisateur* doit avoir un numéro de *téléphone unique.*
  
> [!NOTE]
> **Si vous devez créer deux fois des utilisateurs, la commande échouera, sauf si vous utilisez un autre code de zone ou si les utilisateurs précédents ont été désactivés à l’aide de l'Disable-CsUser cmdlet.**
...
L’outil Stress and Performance de Skype Entreprise Server 2015 peut simuler la fonctionnalité d’extension de liste de distribution (DL) dans le client Skype Entreprise 2015. Vous pouvez ignorer cette étape si vous n’avez pas l’intention d’activer l’extension DL dans l’outil d’approvisionnement des utilisateurs.
...
1. Dans le champ Nombre de listes de **distribution,** indiquez le nombre total de listes de distribution que vous souhaitez créer (il est recommandé de commencer par une valeur qui double le nombre d’utilisateurs dont vous avez.)
...
L’outil Stress and Performance de Skype Entreprise Server 2015 peut également générer des fichiers de configuration factices pour le service Informations d’emplacement. Notez que le service Informations d’emplacement n’a généralement pas d’impact significatif sur les performances sur les serveurs. 
...
Si vous choisissez de tester cette fonctionnalité, remplissez les valeurs du formulaire et cliquez sur le bouton Générer des fichiers de config LIS, qui sera créé. Fichiers CSV appelés :
...
Une fois vos utilisateurs créés (via l’outil de création d’utilisateurs), vous pouvez configurer les profils utilisateur à l’aide de l’outil de configuration de chargement de Skype Entreprise Server 2015 (UserProfileGenerator.exe).
...
Démarrez l’outil Charger la configuration (UserProfileGenerator.exe) et remplissez les onglets. Cet outil crée un répertoire pour chacun des ordinateurs clients dont vous aurez besoin pour exécuter vos simulations. Chaque répertoire client est livré avec un script pour démarrer l’outil Stress and Performance de Skype Entreprise Server 2015 (LyncPerfTool.exe). Les sections ci-dessous donnent des exemples de la façon de remplir les champs sous chaque onglet de l’outil de configuration de chargement de Skype Entreprise Server 2015.
  
> [!IMPORTANT]
> Les valeurs spécifiques à l’utilisateur utilisées dans l’outil De configuration de la charge (UserProfileGenerator.exe) doivent correspondre aux valeurs spécifiées dans l’outil de création d’utilisateurs (UserProvisioningTool.exe) de Skype Entreprise Server 2015 pour le pool. 
...
6. Dans le **champ Domaine de** l’utilisateur, entrez le domaine utilisé pour l’URI SIP. Il est utilisé pour construire l’URI SIP de chaque utilisateur pour se connecter au serveur frontal Skype Entreprise Server 2015 ou au serveur Standard Edition, et peut être différent du domaine de compte.
...
10. Dans le champ Proxy d’accès ou Nom de domaine complet du **pool,** entrez le nom de domaine complet (FQDN) du serveur à qui vous souhaitez que les clients se connectent. Si les utilisateurs se connectent en externe, vous devez taper le proxy d’accès. Si les utilisateurs sont internes, donnez le nom deqdn de leur pool d’entreprise ou serveur Standard Edition.
...
12. Pour le **champ Paramètres** du serveur réseau externe, donnez au proxy d’accès ou au FQDN du pool et, là encore, le **port**. Ces paramètres sont utilisés uniquement pour la simulation de chargement des points de terminaison externes.
...
- **Attendant de conférence -** Fournisse une valeur pour le niveau de charge. Toute valeur autre que Désactivée active le **champ Numéro de** téléphone. Entrez le numéro de téléphone du Standard automatique que vous souhaitez utiliser. Cliquez **sur Avancé** et donnez une valeur au champ **LocationProfile.**
...
- **Paramètres généraux de l’application web :** Cliquez sur **le bouton Paramètres** supplémentaires et définissez **ReachTargetServerUrl** sur l’adresse IP virtuelle (VIP) du pool d’annuaires de l’adresse IP virtuelle du pool frontal.
...
- **Paramètres généraux de mobilité :** Cliquez **sur Paramètres supplémentaires** et définissez le champ UcwaTargetServerUrl sur l’adresse IP virtuelle (VIP) du pool directeur ou l’adresse IP virtuelle du pool frontal.
...
Il est possible de configurer manuellement les plages  de numéro de l’utilisateur en élecliquant sur la case Activer la génération de plage utilisateur personnalisée, puis en double-cliquant sur le scénario du tableau qui possède la plage d’utilisateurs que vous souhaitez personnaliser.
...
Vous devez créer des utilisateurs, des contacts et des scénarios avant d’utiliser l’outil Stress and Performance (LyncPerfTool.exe) de Skype Entreprise Server 2015. Pour plus d’informations sur l’utilisation des outils permettant d’effectuer ces actions, voir Créer des utilisateurs et des [contacts](using-the-tool.md#BKMK_CreateUsersAndContacts) et [configurer](using-the-tool.md#BKMK_UserProfile) le profil utilisateur précédemment dans cet article. L’exécution de ces outils génère également un fichier qui s’exécutera avec l’outil Stress and Performance dans le cadre d’un fichier de lots avec les paramètres requis inclus.
  
### <a name="running-the-skype-for-business-server-2015-stress-and-performance-tool"></a>Exécution de l’outil Stress and Performance de Skype Entreprise Server 2015
...
L’outil Stress and Performance de Skype Entreprise Server 2015 dispose de nombreux compteurs qui peuvent vous aider à comprendre ce que fait le client et s’il rencontre des problèmes.
