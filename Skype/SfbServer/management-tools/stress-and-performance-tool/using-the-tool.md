...
> [!NOTE]
> Load level values for all fields but Local Information Services are **Disabled**, **Low**, **Medium**, **High**, or **Custom**. If you select any setting but Disabled, then configurations are generated for each client. Élevé se traduit par la charge maximale prise en charge sur le serveur ; moyen est de 60 % de la charge élevée ; faible est de 30 %. 
  
- **Instant Messaging -** This includes peer-to-peer and conferencing; choose the appropriate value for Load Level.
    
- **Audio Conferencing -** Choose a load level for audio conferencing *only*  . Appels de peer-to-peer seront traités un peu plus tard dans la section **Scénarios de voix** . Open the **Advanced** tab to enable MultiView.
...
- **Adresse carnet Web requête-** Il s’agit du service de recherche de carnet d’adresses plutôt que le téléchargement de fichiers de carnet d’adresses. If you want to enable this for address book file downloads, click the **Advanced** button and set **EnableABSDownload** to True. Donner une valeur de niveau de charge.
...
For each scenario on the General Scenarios tab (except for Location Information Services), if the value of Load Level is **Custom**, then the conversation rate will be calculated using the corresponding field in the Advanced dialog box. Le nom de champ peut varier selon le scénario, mais la description du champ indique : *Remarque Ce nombre servira uniquement si personnaliser est sélectionné dans le menu déroulant* .
...
Le taux maximal d’utilisateur ouverture de session pour le pool est environ 12 par seconde. This means that you shouldn't start more than 12 LyncPerfTool.exe instances at the same time while users are still signing in. One thousand users will take about 20 minutes to fully sign in at one per second.
...
Each instance of LyncPerfTool.exe running has a separate instance of the counters. Each instance is named by its process ID. If clients are overloaded other issues can occur. Pour éviter ces problèmes :
...
|:-----|:-----|
|Temps en Minutes  <br/> |Time spent since the process was started.  <br/> |
...
|NNN Responses for SetPresence  <br/> |Nombre total de codes de réponse nnn provenant du serveur.  <br/> |
|Appels GetPresence  <br/> |Total number of get presence request attempts.  <br/> |
...
|**Performance Counter**|**Description**|
|:-----|:-----|
|Tentative de téléchargement de fichiers de l’ABS intégral/Delta  <br/> |Nombre total de complet ou delta demandes de téléchargement de fichier lancée.  <br/> |
...
|**Performance Counter**|**Description**|
|:-----|:-----|
|Appels tentées  <br/> |Nombre total de demandes service distribution liste d’extension (DLX) web tentée.  <br/> |
...
|**Compteur de performance**|**Description**|
|:-----|:-----|
|Appels actifs  <br/> |Nombre total d’appels actifs pour l’application de groupe de la réponse.  <br/> |
...
|Calls Received NNN  <br/> |Nombre total de codes de réponse nnn provenant du serveur.  <br/> |
|Messages instantanés reçus/envoyés  <br/> |Nombre total de messages reçus ou envoyés à toutes les sessions.  <br/> |
|Incoming/Outgoing Calls Attempted  <br/> |Total number of incoming/outgoing instant messaging calls attempted.  <br/> |
|Incoming/Outgoing Calls Established  <br/> |Nombre total d’appels de messagerie instantanée entrantes et sortantes établies.  <br/> |
   
**Informations d’appel partage d’application**

|**Compteur de performance**|**Description**|
|:-----|:-----|
|Appels actifs  <br/> |Total number of ongoing incoming/outgoing application sharing calls.  <br/> |
...
|Appels entrants/sortants tentées  <br/> |Nombre total d’appels a tenté de partage d’application entrantes et sortantes.  <br/> |
|Appels entrants/sortants établis  <br/> |Total number of incoming/outgoing application sharing calls established.  <br/> |
   
**CAA Call Information**

|**Performance Counter**|**Description**|
|:-----|:-----|
|Appels actifs  <br/> |Nombre total d’entrantes et sortantes du réseau téléphonique public commuté (RTPC) appelle actuellement en cours.  <br/> |
|Appels s’est arrêtés  <br/> |Nombre total d’appels entrants/sortants par PSTN est déjà arrêté.  <br/> |
|Incoming/Outgoing Calls Attempted  <br/> |Total number of incoming/outgoing PSTN calls attempted.  <br/> |
|Incoming/Outgoing Calls Established  <br/> |Nombre total d’appels de RTPC entrantes et sortantes établies.  <br/> |
   
**Informations sur la conférence**

|**Compteur de performance**|**Description**|
|:-----|:-----|
|Conférences de messagerie instantanées actives  <br/> |Total number of ongoing instant messaging conferences.  <br/> |
|Active Audio/Video Conferences  <br/> |Nombre total d’en cours audio/vidéo (A / V) conférences.  <br/> |
|Conférences de partage d’Application Active  <br/> |Nombre total de conférences qui partage l’application en cours.  <br/> |
|Nombre de Participants  <br/> |Nombre total de participants est actuellement connecté aux conférences.  <br/> |
|Échec de planification de conférence  <br/> |Total number of failures while trying to schedule a conference.  <br/> |
...
|**Compteur de performance**|**Description**|
|:-----|:-----|
|Nombre total de IMMCU joint a réussi  <br/> |Nombre total de conférences de messagerie instantanées est joint.  <br/> |
|Nombre total de DMCU joint a réussi  <br/> |Nombre total de A / vidéoconférences joint.  <br/> |
