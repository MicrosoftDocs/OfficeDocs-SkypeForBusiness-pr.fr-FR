...
2. Vous êtes limité à deux scénarios par étendue. Cependant, le nombre de fournisseurs n’est pas limité. Dans cet exemple, supposons que vous avez créé trois fournisseurs et que vous souhaitez les affecter tous les trois au scénario que vous définissez. Les noms des variables de fournisseur sont LyssProvider, ABServerProvider et SIPStackProvider. Pour définir et affecter plusieurs fournisseurs à un scénario, tapez ce qui suit à une invite de commandes Skype Entreprise Server Management Shell ou Windows PowerShell :
   ...
   ```
    ...
    > [!NOTE]
    > Comme on le sait dans Windows PowerShell, la convention de création d’une table de hachage de valeurs à l’aide `@{<variable>=<value1>, <value2>, <value>...}` est appelée « mise en cache ». Pour plus d’informations sur l’éclaboussure dans Windows PowerShell, consultez [https://go.microsoft.com/fwlink/p/?LinkId=267760](/previous-versions/technet-magazine/gg675931(v=msdn.10)).
...
2. Dans le Windows PowerShell, tapez :
...
3. Pour décharger les modules, tapez :
   ...
   ```
    ...
    > [!TIP]
    > Le déchargement réussi du module vous renvoie à l’invite de commandes Windows PowerShell. Pour confirmer que le module est déchargé, tapez  `Get-Help Edit-CsClsScenario`. Windows PowerShell tente de localiser l’aide de l’applet de commande et échoue.
...
2. Pour ajouter un fournisseur au scénario AlwaysOn, tapez :
   ...
   ```
    ...
    -Loglevel peut être du type Fatal, Error, Warning, Info, Verbose, Debug ou All. -Flags peut être l’un des indicateurs pris en charge par le fournisseur, comme TF_COMPONENT, TF_DIAG. -Les indicateurs peuvent également être de valeur ALL
