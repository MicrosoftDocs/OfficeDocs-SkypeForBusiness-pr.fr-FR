...
description: Cet article explique comment réaliser cette consolidation pour les organisations qui ont des déploiements locaux de Skype Entreprise (ou Lync) qui cherchent à déplacer leur charge de travail UC vers Teams et/ou Skype Entreprise Online.
ms.openlocfilehash: 6a6d433be8b6be5519f1bf7576464603e29ee41b
ms.sourcegitcommit: 9879bc587382755d9a5cd63a75b0e7dc4e15574c
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 07/21/2021
ms.locfileid: "53510725"
---
...
- Il doit y avoir au plus un Microsoft 365 ou Office 365'organisation impliquée. La consolidation dans les scénarios avec plusieurs organisations n’est pas prise en charge.
...
|---------|---------|
|<ul><li>2 déploiements Skype Entreprise sur site dans des forêts AD distinctes<li>Une forêt au maximum est hybride avec Skype Entreprise Online <li> Les orgs sont fédérés les uns avec les autres <li>Les utilisateurs ne sont pas synchronisés entre ces forêts<li> L’organisation peut avoir une organisation Microsoft 365 ou Office 365 et synchroniser son annuaire avec Azure AD</ul>|<ul> <li>1 Microsoft 365 ou Office 365 organisation<li>Plus aucun déploiement local, donc aucun déploiement hybride restant<li>Tous les utilisateurs locaux sont Skype Entreprise Online et peuvent éventuellement être des utilisateurs Teams uniquement <li>Aucun encombrement local de Skype Entreprise n’importe où <li>Les utilisateurs ont toujours une authentification sur site</ul> |
...
1.  Obtenez une Microsoft 365 ou Office 365 organisation si elle n’en existe pas encore.
2.  Assurez-vous que tous les domaines SIP pertinents dans les deux déploiements locaux sont vérifiés Microsoft 365 ou Office 365 domaines.
3.  Choisissez un Skype Entreprise déploiement hybride avec Microsoft 365 ou Office 365. Dans cet exemple, nous allons utiliser OriginalCompany. <span> com.
...
7.  Pour tous les domaines SIP hébergés par d’autres déploiements locaux (dans ce cas, AcquiredCompany. <span> com), [désactivez ces domaines SIP](/powershell/module/skype/disable-csonlinesipdomain) dans Skype Entreprise Online à l’aide `Disable-CsOnlineSipDomain` de PowerShell. (Il s’agit d’une nouvelle fonctionnalité à partir de décembre 2018.)
...
10. Une fois que tous les utilisateurs sont déplacés vers le cloud, désactivez le déploiement hybride Skype Entreprise [déploiement local](cloud-consolidation-disabling-hybrid.md) pour OriginalCompany. <span> com à partir Office 365 :  
    - Désactivez le domaine fractioné dans l’Microsoft 365 ou Office 365 organisation.
...
11. Si ce n’est pas déjà fait, activez [Connecter AAD](cloud-consolidation-aad-connect.md) pour la forêt suivante qui sera hybride (AcquiredCompany. <span> com). À ce stade, l’organisation ressemble **[à la figure C](#figure-c)**. Il peut s’y avoir un autre point de départ courant pour certaines organisations. 
12. Dans PowerShell, [activez les](/powershell/module/skype/enable-csonlinesipdomain) domaines SIP pour le déploiement local suivant qui sera hybride, AcquiredCompany. <span> com. Cette fonctionnalité est effectuée `Enable-CsOnlineSipDomain` à l’aide de la nouvelle fonctionnalité disponible à partir de décembre 2018.
13. Si vous utilisez la fédération fermée, vous devez ajouter des domaines SIP (à l’exception de .microsoftonline.com) du client en ligne pur en tant que domaines autorisés dans le même Microsoft 365 ou \* Office 365.  Notez qu’il peut prendre un certain temps avant que la modification prenne effet et qu’il n’y a aucun risque à le faire tôt, c’est pourquoi nous vous suggérons de le faire bien avant de passer à l’étape 14.
...
- Tous les utilisateurs ont été homed local.  
...
- Certains utilisateurs de l’organisation hybride ont été déplacés vers le cloud (utilisateur A comme indiqué par une ombre en violet). Ces utilisateurs peuvent être soit des utilisateurs Skype Entreprise Online, soit des utilisateurs Teams uniquement avec une interopérabilité totale et une prise en charge de la fédération.<br><br>
    ![Diagramme de la figure B](../media/cloudconsolidationfigb.png)

##### <a name="figure-c"></a>Figure C :

- Tous les utilisateurs d’OriginalCompany. <span> com sont désormais dans le cloud (Skype Entreprise Online). Il est recommandé de les Teams uniquement.
...
- La version sur site est mise à jour pour accepter OriginalCompany. <span> com. (Les certificats de domaine et de périphérie autorisés sont mis à jour).
- L’espace d’adressare SIP partagé est activé entre AcquiredCompany. <span> com et Microsoft 365 ou Office 365 organisation.
...
Les étapes de l’exemple canonique ci-dessus supposent que l’organisation commence par deux déploiements locaux fédérés sans microsoft ni Office 365 présence. Toutefois, certaines organisations peuvent avoir une empreinte Microsoft 365 ou Office 365 existante, et il peut y avoir différents points d’entrée dans la séquence ci-dessus. Il existe quatre configurations classiques :

- Plusieurs organisations locales fédérées sans Microsoft 365 ou Office 365 organisation. Dans ce cas, commencez à l’étape 1.
...
- Une organisation hybride qui se fédéré avec une ou plusieurs autres organisations purement locales, dont aucune n’est synchronisée avec AAD. Une telle organisation ressemblerait à l’organisation hypothétique dans **la figure E**, présentée ci-dessous.
    ...
    - Ces organisations doivent :
        ...
        - Si vous souhaitez synchroniser d’autres forêts Skype Entreprise avec AAD avant de terminer la migration de l’organisation hybride, l’organisation doit effectuer l’étape 7 (désactiver tous les domaines SIP en ligne dans tout autre déploiement Skype Entreprise local qui sera synchronisé avec AAD), puis activer AAD Connecter, puis ne passer à l’étape 10 (désaffecter le déploiement hybride d’origine).       
                **Figure E**<br>
                ![Diagramme de la figure E](../media/cloudconsolidationfige.png)
- Une organisation Skype Entreprise Online (qui peut ou non utiliser Teams) qui se fédéré avec une organisation Skype Entreprise distincte. Une fois que cette organisation a désactivé le domaine SIP en ligne pour l’organisation sur site et activé AAD Connecter pour l’organisation Skype Entreprise sur site, elle ressemble à l’organisation hypothétique présentée dans la **[figure C](#figure-c)** qui a effectué les étapes 1 à 11.

## <a name="limitations"></a>Limites

- Il doit y avoir au plus un Microsoft 365 ou Office 365'organisation impliquée. La consolidation dans les scénarios avec plusieurs organisations n’est pas prise en charge.
- Une seule forêt Skype Entreprise local peut être en mode hybride (espace d’adressas SIP partagé) à la fois. Toutes les autres forêts Skype Entreprise sur site doivent rester purement en local et être fédérées les unes avec les autres et avec l’Microsoft 365 ou Office 365 organisation.
...
Lorsque vous déplacez des utilisateurs de l’environnement local vers le cloud dans un environnement hybride, vous pouvez les déplacer vers le mode Skype Entreprise uniquement ou TeamsOnly. *Si vous prévoyez de déplacer des utilisateurs vers le mode TeamsOnly, veillez à lire d’abord cette section.*

- Lorsque vous affectez le mode TeamsOnly à un utilisateur, toutes les conversations et tous les appels d’un autre utilisateur se viennent du client Teams utilisateur. 
