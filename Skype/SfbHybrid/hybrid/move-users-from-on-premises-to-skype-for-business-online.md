...
> [!NOTE]
> En prévision du retrait à venir de Skype Entreprise Online, Microsoft simplifiera prochainement la façon dont les organisations passeront à Teams et il ne sera plus possible de passer à Skype Entreprise Online.  Une fois ces modifications disponibles, lors du déplacement d’un utilisateur de l’local vers le cloud, le mode TeamsOnly est automatiquement affecté aux utilisateurs et leurs réunions à partir de l’ordinateur local sont automatiquement converties en réunions Teams, comme si le commutateur avait été spécifié, que le commutateur soit réellement spécifié `-MoveToTeams` ou non. Nous prévoyons de publier cette fonctionnalité avant le retrait réel du 31 juillet 2021.   Actuellement, si ce commutateur n’est pas spécifié, les utilisateurs passent de l’accueil dans Skype Entreprise Server local à Skype Entreprise Online, et leur mode reste inchangé, ce qui est la fonctionnalité documentée dans cet article.
...
La séquence de cmdlet suivante peut être utilisée pour déplacer un utilisateur vers Skype Entreprise Online. Il suppose que les informations d Microsoft 365 sont un compte distinct et fournies en tant qu’entrées pour Get-Credential invite.
