...
Skype Entreprise Online utilise les fonctionnalités de l’infrastructure à clé publique dans le système d’exploitation Windows Server afin de protéger les données de clé utilisées pour le chiffrement des connexions TLS (Transport Layer Security). Les clés utilisées pour le chiffrement multimédia sont échangées via des connexions TLS. 
...
Cette section fournit une vue d’ensemble des éléments fondamentaux qui constituent le cadre de sécurité de Microsoft SfBO. Ces éléments qui le constituent sont les suivants :
