...
1. Une conférence Skype entreprise est hébergée dans la même région que celle de l’organisateur de la Conférence. Cela s'applique au cloud Office 365 si l'organisateur est un utilisateur en ligne, ou à un centre de données sur site si l'organisateur de la réunion est un utilisateur sur site.
...
Pour obtenir davantage de détails sur la sélection du chemin emprunté par les médias, veuillez vous reporter à la rubrique [ICE - Connectivité Edge des médias](https://aka.ms/AVEdge). Même si cette vidéo est sur Lync Server 2013, les principes et protocoles s’appliquent toujours à Skype entreprise.
...
Le trafic ne traversera pas la connexion ExpressRoute si les deux points de terminaison sont en mesure de communiquer directement sur Internet ou s'ils sont tous les deux situés dans votre réseau. Cela inclut les éléments multimédias pour les appels d’égal à égal, le trafic provenant d’Internet destiné à un déploiement local, ou tout trafic entre les serveurs Microsoft Edge et Internet et 365. À titre d'exemple, il peut s'agir du trafic d'un utilisateur rejoignant une conférence en ligne depuis un hôtel.
...
Les flux d'appels hybrides sont utilisés lorsque vous disposez d'un déploiement Skype Entreprise comprenant au moins plusieurs utilisateurs domiciliés sur site. Les flux d’appels dans cette section incluent des conférences locales et des appels d’égal à égal ou PSTN avec au moins un utilisateur hébergé sur site.
