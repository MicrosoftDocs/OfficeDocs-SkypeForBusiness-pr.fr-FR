...
Pour plus d’informations, [voir Configure a multi-forest environment for hybrid Skype for Business](../../SfbServer/skype-for-business-hybrid-solutions/deploy-hybrid-connectivity/configure-a-multi-forest-environment-for-hybrid.md).
...
Si c’est ce que vous cherchez et si vous avez un grand nombre de domaines SIP (ce qui rendrait l’ajout de SANS plus coûteux), vous pouvez configurer votre proxy inverse pour utiliser HTTP pour la demande initiale du service de découverte automatique, au lieu d’utiliser HTTPS (qui est la configuration par défaut). [L’article Plan for Mobility](../../SfbServer/plan-your-deployment/mobility.md) (Planifier la mobilité) vous présente davantage d’informations à ce sujet.
...
> [!Note]
> **Pourquoi la dernière version de Windows ?** Il se peut que les versions antérieures ne disposent pas des autorisations permettant d’activer toutes les fonctionnalités. Vous pouvez utiliser l’administrateur de cluster pour créer les partages de fichiers. Pour plus d’informations, consultez cet article de support technique sur la création de partages de fichiers sur un [cluster.](https://support.microsoft.com/help/224967)
