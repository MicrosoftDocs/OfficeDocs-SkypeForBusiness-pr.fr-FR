...
Les clients qui utilisent l’une des fonctionnalités suivantes du service Exchange Unified Messaging Online sont affectés :
...
Familiarisez-vous avec l’annonce [du blog](https://blogs.technet.microsoft.com/exchange/2019/02/08/retiring-unified-messaging-in-exchange-online/) et cet article pour planifier une migration fluide pour vos utilisateurs. Pour [plus d’informations sur](https://support.office.com/article/check-skype-for-business-voicemail-and-options-2deea7f8-831f-4e85-a0d4-b34da55945a8) les fonctionnalités de messagerie vocale du système téléphonique, voir Vérifier la messagerie vocale Skype Entreprise et les options.  

### <a name="establish-a-skype-for-business-hybrid-topology"></a>Établir une topologie hybride Skype Entreprise

Si aucune topologie hybride Skype Entreprise n’est établie, vous devez le faire pour permettre une migration fluide de vos utilisateurs de messagerie vocale. Pour [plus d’informations, voir Configurer Skype](../../SfbHybrid/hybrid/configure-federation-with-skype-for-business-online.md) Entreprise hybride.

> [!Note]
> Il n’est pas nécessaire de migrer vos utilisateurs en ligne pour la migration du service de messagerie vocale. Toutefois, pour que les utilisateurs locaux tirent parti du service de messagerie vocale du système téléphonique, une topologie hybride doit être établie.
...
1. Basculez vos numéros de téléphone des standard automatiques UMO Exchange vers les nouveaux standard automatiques.
...
| VM | Fonctionnalités utilisateur | Accès à la numérotation PSTN  | Accès abonné  | N | v    |
| VM | Fonctionnalités utilisateur | Délégué  | e-mail d’appel manqué  | N | v    |
| VM | Fonctionnalités utilisateur | PSTN Outlook Voice Access   | Accès abonné  | N | v    |
...
| VM | Fonctionnalités utilisateur | Enregistrer des salutations personnelles (PSTN) : lire sur le téléphone |  | N | v    |
...
| VM | Fonctionnalités utilisateur | Indicateur de message en attente (MWI) à l’aide de messages notifiés SIP |  | N | v    |
...
| VM | Fonctionnalités utilisateur | Envoi d’un message vocal à un groupe d’utilisateurs  |Diffusion de messagerie vocale   | N | v   |
| VM | Fonctionnalités utilisateur | Notification de messagerie vocale à l’aide de SMS    | Les utilisateurs peuvent recevoir un SMS lorsqu’ils ont un nouveau message vocal    | N | v    |
...
| VM | Fonctionnalités utilisateur | Lire sur le téléphone (PSTN) : lire un message | M’appeler sur ma cellule pour écouter le message vocal  | N | v    |
...
| UM | Fonctionnalités utilisateur | Indicateur de message en attente (MWI) sur les téléphones certifiés Skype Entreprise   |Peut être fourni par un partenaire téléphonique  | Non | Oui    |
| AA | Fonctionnalités de service | AA support 3rd-party PBX    |  | N | v    |
| AA | Fonctionnalités de service | Prise en charge de Skype Entreprise Server   |  | v | v    |
| AA | Fonctionnalités de service | Prise en charge de Microsoft Teams|  | v | N    |
...
| AA | Fonctionnalités de service | Transférer vers le numéro PSTN en externe  |  | Consultez la section Problèmes connus ci-dessous | v    |
...
| Fax| Fonctionnalités de service | Intégration des télécopies|  | N | v    |
...
Pour vérifier que vos utilisateurs ont été migrés vers la messagerie vocale cloud, laissez une messagerie vocale à un utilisateur et vérifiez le corps du message dans Outlook. Les messages vocaux cloud ont un pied de groupe qui lit :
...
- Si votre organisation a une obligation légale ou de conformité de désactiver la transcription pour les utilisateurs, assurez-vous qu’elle est désactivée après la migration. Pour plus d’informations, voir [Configurer la messagerie vocale cloud.](/microsoftteams/set-up-phone-system-voicemail)
- Si vous avez précédemment configuré des stratégies et des règles de la VM Exchange, assurez-vous qu’elles sont efficaces.
- Familiarisez-vous avec les cmdlets PowerShell du service de messagerie vocale cloud pour modifier les paramètres utilisateur.  
...
|Règles Outlook pour les messages de la VM | Aucune modification<br>Les règles créées précédemment s’appliquent aux messages DE CVM après la migration.
 |

### <a name="user-management-and-provisioning-in-cvm"></a>Gestion et approvisionnement des utilisateurs dans la gestion des cvm

Les nouveaux utilisateurs de Skype Entreprise seront automatiquement mis en service pour la messagerie vocale cloud lors de leur création. Aucune licence ou travail d’administrateur supplémentaire n’est requis pour mettre en service de nouveaux utilisateurs de messagerie vocale. Voir [Configurer la messagerie vocale cloud](/microsoftteams/set-up-phone-system-voicemail) pour en savoir plus sur la gestion des stratégies pour les utilisateurs existants et nouveaux.
