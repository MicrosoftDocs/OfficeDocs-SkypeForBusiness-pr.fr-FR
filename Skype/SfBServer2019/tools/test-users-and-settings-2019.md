...
Identity : atl-cs-001.litwareinc.com TestUsers : {sip :watcher1@litwareinc.com, sip :watcher2@litwareinc.com...} ExtendedTests : {TestUsers = IList<System. String> ; Nom = test PSTN ; Te...} TargetFqdn : atl-cs-001.litwareinc.com numéro_port : 5061To Vérifiez que le nœud observateur a été correctement configuré, tapez la commande suivante à partir de Skype entreprise Server Management Shell :
...
- La clé de registre requise est créée (terminée lors de l’exécution de la cmdlet Set-applet cswatchernodeconfiguration)
    
- Vos serveurs exécutent la version correcte de Skype entreprise Server
    
- Vos ports sont configurés correctement
    
- Vos utilisateurs de test affectés disposent des informations d’identification requises.
...
BITSAdmin est déconseillé et n’est pas forcément disponible dans les versions ultérieures de Windows. Les outils d’administration du service BITS sont désormais fournis par les cmdlets PowerShell BITS.
  
Paramètres de proxy Internet pour le compte NetworkService défini sur NO_PROXY. 
...
> [!NOTE]
> Les passerelles XMPP et les proxys sont disponibles dans Skype entreprise Server 2015, mais ne sont plus pris en charge dans Skype entreprise Server 2019. Pour plus d’informations, consultez la rubrique migration de la [Fédération XMPP](../migration/migrating-xmpp-federation.md) .
...
> [!NOTE]
> : Ne faites pas précéder le nom de la variable du caractère $. Utilisez un nom de variable comme RegistrationTest (non $RegistrationTest). 
  
Lorsque vous exécutez cette commande, vous obtiendrez une sortie semblable à ceci :
  
Nom de domaine complet cible : résultat atl-cs-001.litwareinc.com : latence des échecs : message d’erreur 00:00:00 : cet ordinateur ne dispose d’aucun certificat attribué. Diagnostic : vous pouvez accéder à des informations bien plus détaillées sur ce problème que le message d’erreur affiché ici. Pour accéder à ces informations au format HTML, utilisez une commande semblable à celle-ci pour enregistrer les informations stockées dans la variable RegistrationTest dans un fichier HTML :
