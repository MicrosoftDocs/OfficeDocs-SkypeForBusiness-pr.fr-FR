...
Les articles de cette section s’appliquent à toutes les options de connexion au réseau téléphonique commuté (RTC) public : forfait d’appels, Operator Connect et routage direct. Pour plus d’informations sur toutes les options de connectivité RTC, consultez [les options de connectivité RTC](pstn-connectivity.md).

Si vous choisissez Forfait d’appels ou Operator Connect, la plupart du routage des appels est géré par Microsoft ou votre fournisseur. Toutefois, le routage direct nécessite des étapes supplémentaires pour configurer le routage des appels. 
...
N’oubliez pas que vous pouvez affecter une stratégie de routage vocal en ligne de routage direct au plan d’appel et Operator Connect utilisateurs. Vous souhaiterez peut-être effectuer cette opération, par exemple, pour permettre aux utilisateurs de se connecter directement à un centre d’appels. Vous pouvez configurer une jonction de routage direct vers le centre d’appels.
