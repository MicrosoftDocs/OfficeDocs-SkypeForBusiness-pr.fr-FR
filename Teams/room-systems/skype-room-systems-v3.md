...
Pour plus d’informations sur l’état de l’application Microsoft équipes salles et les périphériques connectés sont écrits par l’application Microsoft équipes salles dans le journal des événements Windows et expliquées dans [comprendre les entrées du journal](azure-monitor-manage.md#understand-the-log-entries). 
...
Nous recommandons d’utiliser Microsoft Operations Manager Suite à surveiller vos systèmes salles d’équipes Microsoft. Pour obtenir des instructions sur la configuration de surveillance et les alertes de base, voir [gestion de déployer Microsoft équipes salles avec Azure moniteur](azure-monitor-deploy.md). 
