...
Le rapport de routage direct PSTN du CQD se présente sous quatre sections :
  ...
  - [Vue d’ensemble du PSTN](#pstn-overview)
...
## <a name="pstn-overview"></a>Vue d’ensemble du PSTN
...
| 5            | La principale raison de la fin des appels a affecté la qualité du service au cours des 180 derniers jours. Vous trouverez des détails sur l’état du service sur la page Network Effective Ratio (NER).                    |
...
Cette page présente les tendances d’utilisation des services par jour et le détail des commentaires des utilisateurs par géographie.
  ...
  - **Commentaires des utilisateurs –** La note « Évaluer mon appel » provient de l’utilisateur. Le 3-5 est considéré comme un bon appel. Le 1-2 est considéré comme un appel mauvais.
...
Il s’agit de la même valeur qui apparaît dans le tableau de bord État global. Vous pouvez vérifier toutes les heures le numéro de la zone NER avec les détails des appels affectés pour les deux itinéraires d’appel (entrant/sortant) sur le rapport d’efficacité du réseau toutes les heures et le graphique de raison de fin d’appel ci-dessous.
  ...
  - **NER** - Possibilité (%) d’un réseau qui peut fournir des appels en mesurant le nombre d’appels envoyés et le nombre d’appels remis à un destinataire.

  - **Code de réponse SIP**- Le statut d’un code de réponse à un nombre de trois chiffres indique le statut de l’appel.
...
[Utiliser Power BI pour analyser les données du CQD pour Microsoft Teams](CQD-PSTN-report.md)
