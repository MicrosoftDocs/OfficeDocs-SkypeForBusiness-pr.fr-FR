...
- [Définissez l’URL du serveur de provisionation de passerelle SIP](#set-the-sip-gateway-provisioning-server-url).
...
3. Sous **Paramètres** ou **Paramètres avancés** de l’utilitaire web, entrez l’URL du serveur de mise en service indiquée ci-dessus.
...
2. **Sélectionnez Teams appareilsSIP** > .

3. En haut à droite, sélectionnez **Appareils** **ActionsProvision** >  et suivez l’une des étapes suivantes :
  ...
  - **Pour mettre en service un appareil :**
   ...
   - **Pour mettre en service plusieurs appareils :**
     ...
     b. Dans le **volet Périphériques** d’approvisionnement, **sous Télécharger plusieurs adresses MAC**, **sélectionnez Télécharger un modèle**.
     
     c. **EnregistrezTemplate_Provisioning.csv** sur votre ordinateur et remplissez les champs **ID MAC** **et** Emplacement.
    
     d. Dans le **volet Périphériques** de provision, **sélectionnez Télécharger plusieurs adresses MAC**. 

     e. À droite du Télécharger d’adresses **MAC**, sélectionnez un **fichier, puis****Template_Provisioning.csvfichier contenant** vos données.

     f. Dans le volet **Périphériques** d’approvisionnement, sous En attente **d’activation**, sélectionnez un appareil, puis Générer du **code** de vérification pour générer un code de vérification à une seule heure pour chaque périphérique mis en service. Notez le code de vérification pour chaque appareil SIP.
...
5.  Dans le **volet Provision des appareils**, sous **En attente de la sign in**, **sélectionnez Signed out(1)).**

6. Dans la **boîte de dialogue Se connectez à** un utilisateur, copiez ou notez le code de coupage de l’appareil SIP.

7. Allez à [https://microsoft.com/devicelogin](https://microsoft.com/devicelogin), puis sous **Entrer le code**, entrez le code d’appariment de l’appareil SIP, puis sélectionnez **Suivant**.
...
Pour vous sortant d’un appareil sur le Centre Teams’administration :
...
Vous pouvez afficher et surveiller l’inventaire de vos appareils SIP dans le Teams d’administration une fois que les utilisateurs des appareils se sont connectés au moins une fois. Voici comment procéder :

1. Connectez-vous au [Teams d’administration.](https://admin.teams.microsoft.com/)

2. **Sélectionnez Teams appareilsSIP** > . Tous les appareils SIP connectés sont répertoriés sur la droite.
