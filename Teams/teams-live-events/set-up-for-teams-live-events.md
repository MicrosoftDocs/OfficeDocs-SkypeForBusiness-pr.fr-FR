...
## <a name="step-1-set-up-your-network-for-live-events-in-microsoft-teams"></a>Étape 1: configuration de votre réseau pour les événements en direct dans Microsoft teams
Les événements dynamiques de démarrage rapide requièrent [la préparation du réseau de votre organisation à Microsoft teams](https://docs.microsoft.com/microsoftteams/prepare-network).  
...
|**Autoriser la planification**     |L’activation de cette option permet aux utilisateurs de votre organisation de créer et de planifier des événements en direct dans Teams. Il est important de savoir que si vous souhaitez que les utilisateurs planifient des événements en direct d’encodeur externe, vous devez effectuer des étapes supplémentaires. Pour en savoir plus, voir [autoriser les utilisateurs à planifier des événements d’encodeur externe](#enable-users-to-schedule-external-encoder-events).     |
|**Autoriser la transcription pour les participants** (bientôt disponible) |Ce paramètre ne peut être appliqué qu’aux événements de démarrage rapide. L’activation de cette fonction permet aux participants en temps réel d’afficher des légendes et des traductions en temps réel lors de l’événement.         |
...
### <a name="enable-users-to-schedule-external-encoder-events"></a>Permettre aux utilisateurs de planifier des événements de codeur externes

Pour permettre aux utilisateurs de planifier des événements d’encodeur externe, vous devez également procéder comme suit:

1. Activez Microsoft Stream pour les utilisateurs de votre organisation. Microsoft Stream est disponible dans le cadre des abonnements Office 365 éligibles ou d’un service autonome. Microsoft Stream n’est pas inclus dans les offres Business Essentials et Business Premium. Pour plus d’informations, voir [vue d’ensemble des licences de flux](https://docs.microsoft.com/stream/license-overview) .

      Apprenez-en davantage sur la façon dont vous pouvez [attribuer des licences aux utilisateurs dans Office 365 de](https://support.office.com/article/Assign-licenses-to-users-in-Office-365-for-business-997596B5-4173-4627-B915-36ABAC6786DC) sorte que les utilisateurs puissent accéder à Microsoft Stream. Assurez-vous que Microsoft Stream n’est pas bloqué pour les utilisateurs tels que définis dans [cet article](https://docs.microsoft.com/stream/disable-user-organization).

2. Assurez-vous que les utilisateurs ont une autorisation de création d’événements en direct dans Microsoft Stream. Par défaut, les administrateurs peuvent créer des événements en direct d’encodeur externe. L’administrateur Microsoft Stream peut [activer des utilisateurs supplémentaires pour la création d’événements en direct](https://docs.microsoft.com/stream/live-event-administration#enabling-and-restricting-users-to-creating) dans Stream.  

3. Assurez-vous que les organisateurs d’événements en direct peuvent être envoyés à la stratégie d’entreprise définie par l’administrateur de flux. Si un administrateur de Microsoft Stream a [configuré une stratégie d’instructions de société](https://docs.microsoft.com/stream/company-policy-and-consent) et nécessite l’acceptation des employés de cette politique avant d’enregistrer le contenu, les utilisateurs doivent le faire avant de créer un événement en direct (avec une production d’encodeur externe) dans Teams. Avant de déployer la fonctionnalité événements en direct au sein de l’organisation, assurez-vous que les utilisateurs qui créeront ces événements en direct pourront être envoyés à la stratégie. 
...
Les solutions SDN/eCDN suivantes sont déjà intégrées et peuvent être configurées pour être utilisées avec Microsoft Stream.
