...
ms.locfileid: "52947576"
---
# <a name="configure-operator-connect"></a>Configurer Operator Connect

>[!NOTE]
>L’opérateur Connect est actuellement disponible uniquement en **prévisualisation publique.** La prévisualisation publique vous permet de tester les fonctionnalités à venir et de fournir des commentaires. Les fonctionnalités incluses dans la prévisualisation publique peuvent ne pas être complètes, subi des modifications et ne sont pas pris en charge dans Office 365 Government Clouds.

Cet article décrit comment configurer Operator Connect. Avant de configurer Operator Connect, assurez-vous de lire [Plan pour Operator Connect](operator-connect-plan.md) pour plus d’informations sur les conditions préalables et les licences.

## <a name="enable-an-operator"></a>Activer un opérateur

Vous pouvez activer, modifier et supprimer des opérateurs dans le Centre d’administration Teams. Dans le volet de navigation gauche, allez à **Voice > Opérateurs.**
...
3. **Fournir des informations de contact (facultatif).** Si vous souhaitez que les opérateurs vous contactent pour obtenir des informations supplémentaires sur l’opérateur Connect, cochez la case et fournissez vos informations de contact.  
...
- Si vous devez acquérir des numéros de téléphone pour de nouveaux utilisateurs, [consultez Acquérir les numéros pour les nouveaux utilisateurs de Teams.](#acquire-numbers-for-new-teams-users)

- Si vous voulez déplacer des numéros existants des forfaits d’appels vers l’opérateur Connect, voir Déplacer les numéros des plans d’appels [vers l’opérateur Connect.](#move-numbers-from-calling-plans-to-operator-connect)

- Si vous voulez déplacer des numéros existants du routage direct vers l’opérateur connect, voir Déplacer les numéros du routage direct [vers l’opérateur connect.](#move-numbers-from-direct-routing-to-operator-connect)

>[!IMPORTANT]
>**Adresses d’urgence :** Les numéros de téléphone associés à des adresses de secours sont gérés par votre opérateur. Une fois que vous avez créé des adresses d’urgence dans le Centre d’administration Teams, votre opérateur attribue des numéros de téléphone à ces adresses d’urgence. Pour modifier les adresses d’urgence et les numéros de téléphone qui leur sont affectés, contactez votre opérateur.

### <a name="acquire-numbers-for-new-teams-users"></a>Acquérir des numéros pour les nouveaux utilisateurs de Teams

Pour acquérir des numéros pour les nouveaux utilisateurs de Teams, suivez les étapes suivantes :

1. **Attribuer une licence Phone System.** Vous pouvez attribuer une licence Phone System à vos utilisateurs à partir du Centre d’administration Microsoft 365 ou à l’aide de PowerShell. Pour plus d’informations, [voir Attribuer des licences de module complémentaire Teams aux utilisateurs.](teams-add-on-licensing/assign-teams-add-on-licenses.md)

2. **Assurez-vous d’être en mode Teams uniquement.** Pour vérifier si votre organisation est en mode Teams uniquement, dans le Centre d’administration Teams, allez dans les paramètres à l’échelle de **l’organisation > mise à niveau de Teams.** Pour vérifier si un utilisateur est en  mode Teams uniquement, sélectionnez un compte d’utilisateur dans Utilisateurs. Dans **l’onglet** Compte, sous **Mise à niveau de Teams,** vérifiez que votre mode de coexistence est définie sur « Teams uniquement ».

3. **Créer et valider des adresses de secours.** Dans le Centre d’administration Teams, allez à **Emplacements > d’urgence** pour configurer des adresses de secours. Pour en savoir plus, [consultez Ajouter, modifier ou supprimer un emplacement d’urgence pour votre organisation.](add-change-remove-emergency-location-organization.md)

4. **Acquérir des nombres.** Allez sur le site web de votre opérateur pour commander et acquérir des numéros de téléphone. Pour obtenir une liste des sites web d’opérateurs, voir [Opérateurs.](#operators) Vous devez fournir votre ID de locataire. Si vous ne connaissez pas votre ID de locataire, consultez Trouver votre ID de locataire [Microsoft 365](/onedrive/find-your-office-365-tenant-id) pour plus d’informations.

5. **Attribuer des nombres.** Une fois la commande terminée par votre opérateur, celui-là charge les numéros vers votre client. Vous pouvez afficher les numéros et le fournisseur dans le Centre d’administration Teams en vous voyant sur Les numéros **de > De voix.** Attribuez des numéros à des utilisateurs à l’aide du Centre d’administration Teams ou de PowerShell. Pour plus d’informations, voir [Attribuer des numéros.](#assign-numbers)
 

### <a name="move-numbers-from-calling-plans-to-operator-connect"></a>Déplacer des numéros des forfaits d’appels vers l’opérateur Connect

1. Contactez votre opérateur pour le portage de vos numéros vers l’opérateur Connect. Consultez [les](#operators) opérateurs pour trouver le site web de votre opérateur.
...
3. Affectez des numéros d’opérateur Connect aux utilisateurs à l’aide du Centre d’administration Teams ou de PowerShell. Pour plus d’informations, voir [Attribuer des numéros.](#assign-numbers)

 
### <a name="move-numbers-from-direct-routing-to-operator-connect"></a>Déplacer des nombres du routage direct vers l’opérateur Connect
...
4. Une fois la commande terminée par votre opérateur, celui-là charge les numéros vers votre client. Vous pouvez afficher les numéros et le fournisseur dans le Centre d’administration Teams en vous voyant sur Les numéros **de > De voix.** Affectez des numéros d’opérateur Connect aux utilisateurs à l’aide du Centre d’administration Teams ou de PowerShell. Pour plus d’informations, voir [Attribuer des numéros.](#assign-numbers)

   

### <a name="assign-numbers"></a>Attribuer des numéros

Que vous ajoutiez de nouveaux utilisateurs de Teams ou que vous entagiez des utilisateurs existants à l’opérateur Connect, la procédure d’attribution de numéros est la suivante :

Pour attribuer des numéros à l’aide du Centre d’administration Teams, allez sur **Numéros de téléphone.** Les étapes sont identiques à l’affectation de numéros pour les forfaits d’appels. Pour plus d’informations, [voir Attribuer un numéro de téléphone à un utilisateur.](assign-change-or-remove-a-phone-number-for-a-user.md)
