...
- [Créer un package de mise en service pour Windows 10](/windows/configuration/provisioning-packages/provisioning-create-package)
...
| `<CoordinatedMeetings>` | `<TrustedAccounts>`                       |           | Il s’agit d’une liste séparée par des virgules des upns pour chaque appareil Teams Room ou Surface Hub à partir duquel l’appareil doit accepter les demandes de réunion ou vers laquelle les demandes de réunion doivent être envoyées.<br>Valeurs acceptées : chaîne                                                                                                                                                                                                                                                                                                                         |
...
|                         |                                           | `default` | Détermine le périphérique sur lequel le microphone sera actif lorsqu’une réunion débute. Un seul appareil (généralement un appareil Salles d’équipe) peut avoir ce champ définie, tandis que le champ de ce champ doit être utilisé pour éviter l’écho et les commentaires audio pour les autres `true` `false` appareils.<br>Valeurs acceptées `true` : , `false`                                                                                                                                                                                                           |
...
|                         |                                           | `default` | Détermine le périphérique sur lequel l’appareil photo sera actif lorsqu’une réunion commence. Pour une expérience meilleure, nous vous recommandons de définir uniquement l’appareil Salles d’équipe lorsque tous les autres `true` appareils sont réglés sur `false` .<br>Valeurs acceptées `true` : , `false`                                                                                                                                                                                                                                                                  |
...
Pour finir, après avoir créé le package d’approvisionnement, appliquez le package d’approvisionnement à votre Surface Hub :
...
Après avoir supprimé le profil de configuration qui contenait les paramètres de réunion coordonnée pour votre Surface Hub, utilisez les étapes suivantes pour réinitialiser l’application Teams sur le Surface Hub :
