...
- La **sécurité basée sur la virtualisation** Windows les informations d’identification dérivées NTLM et Kerberos et d’autres secrets s’exécutent dans un environnement protégé isolé du système d’exploitation en cours d’exécution.
...
Le compte Administration n’est pas requis pour le bon fonctionnement des appareils salles Teams et peut être renommé ou même supprimé. Toutefois, avant de supprimer le compte Administration, veillez à configurer un autre compte d’administrateur local configuré avant de supprimer celui fourni avec salles Teams appareils. Pour plus d’informations sur la modification d’un mot de passe pour un compte Windows local à l’aide d’outils de Windows intégrés ou de PowerShell, consultez les rubriques suivantes :
...
Vous pouvez également importer des comptes de domaine dans le groupe administrateur Windows local. Vous pouvez le faire pour les comptes Azure AD à l’aide de Intune. Pour plus d’informations, consultez [Policy fournisseur de solutions Cloud – RestrictedGroups.](/windows/client-management/mdm/policy-csp-restrictedgroups)
...
- **Microsoft Store** [conditions préalables pour l’Microsoft Store pour Entreprises et l’éducation](/microsoft-store/prerequisites-microsoft-store-for-business#proxy-configuration)
...
Pour optimiser les performances des médias en temps réel, nous vous recommandons vivement de configurer Teams trafic multimédia pour contourner les serveurs proxy et autres appareils de sécurité réseau. Les médias en temps réel sont très sensibles à la latence et les serveurs proxy et les périphériques de sécurité réseau peuvent dégrader considérablement la qualité vidéo et audio des utilisateurs. En outre, étant donné que Teams média est déjà chiffré, il n’y a aucun avantage tangible à passer le trafic via un serveur proxy. Pour plus d’informations, consultez [Mise en réseau (vers le cloud) : point de vue d’un architecte](/microsoft-365/solutions/networking-design-principles?view=o365-worldwide) qui traite des recommandations réseau pour améliorer les performances des médias avec Microsoft Teams et Salles Microsoft Teams.
...
La jointure de proximité et d’autres fonctionnalités de salles Teams s’appuient sur Bluetooth. Toutefois, l’implémentation Bluetooth sur salles Teams appareils n’autorise pas une connexion d’appareil externe à un appareil salles Teams. Bluetooth’utilisation de la technologie sur les appareils salles Teams est actuellement limitée aux balises publicitaires et aux connexions proximales invités. Le `ADV_NONCONN_INT` type d’unité de données de protocole (PDU) est utilisé dans la balise de publicité. Ce type PDU est destiné aux appareils non connectables qui annoncent des informations sur l’appareil d’écoute. Il n’existe aucune Bluetooth appairage d’appareils dans le cadre de ces fonctionnalités. Pour plus d’informations sur les protocoles Bluetooth, consultez le [site web Bluetooth SIG](https://www.bluetooth.com/blog/bluetooth-low-energy-it-starts-with-advertising/).
