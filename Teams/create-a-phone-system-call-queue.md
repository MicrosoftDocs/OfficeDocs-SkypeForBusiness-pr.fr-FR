...
Lorsque quelqu’un appelle un numéro de téléphone associé à une file d’attente d’appels via un [compte de ressources](manage-resource-accounts.md): 
1. Ils entendent un message d’accueil (le cas échéant) 
2. Son appel est placé dans la file d’attente pour attendre l’agent d’appel suivant. 
...
Si vous configurez plusieurs standards automatiques, il est possible que vous ne deviez affecter un numéro de téléphone au compte de ressources principal du standard automatique, qui peut diriger les appelants vers vos files d’attente d’appels ou des standards automatiques imbriqués. Dans ces situations, vous devez créer tous les standards automatiques et les files d’attente d’appels dans votre système sans affecter d’options de clavier numérique, puis modifier les paramètres par la suite. Cela est nécessaire, car vous n’êtes pas autorisé à créer une option de liaison vers une file d’attente d’appels ou un standard automatique qui n’existe pas encore.
