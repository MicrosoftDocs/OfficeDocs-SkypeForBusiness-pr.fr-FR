...
Pour une organisation qui n’a pas encore commencé à utiliser Teams, l’administrateur doit changer le mode à l’échelle du client d’Islands à SfbWithTeamsCollab. (Pour les organisations qui ont déjà une Teams d’utilisation, l’administrateur doit « inexer » les utilisateurs déjà actifs dans Teams pour s’assurer que cette modification ne s’applique pas à eux. Pour plus d’informations, consultez une méthode de sélection des fonctionnalités pour une organisation qui utilise déjà [Teams mode Îles.)](upgrade-to-teams-on-prem-implement.md#a-select-capabilities-upgrade-for-an-organization-that-is-already-using-teams-in-islands-mode) 
