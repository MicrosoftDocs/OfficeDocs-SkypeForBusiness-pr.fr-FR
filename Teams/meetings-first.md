---
title: Réunions d’abord - Microsoft Teams
author: CarolynRowe
ms.author: crowe
manager: serdars
ms.topic: conceptual
...
Les réunions d’abord s’appliquent aux organisations qui utilisent Skype Entreprise Server avec Téléphonie –  Grandes entreprises qui souhaitent accélérer leur déplacement vers les réunions Teams. Meetings First est particulièrement destiné aux organisations dotées d’une discipline informatique forte qui souhaitent un chemin de mise à niveau managé vers Teams.
...
- Expérience utilisateur rationalisée dans Teams, centrée sur les réunions teams et les équipes et les canaux (qui peuvent être masqués à l’aide de la [stratégie d’autorisations d’application](teams-app-permission-policies.md)). Les [conversations privées Teams, les appels et la présence autonome](teams-client-experience-and-conformance-to-coexistence-modes.md) ne sont pas exposés dans Meetings First, ce qui permet au déploiement et à l’adoption de se concentrer sur les réunions.
...
Pour bénéficier de la meilleure expérience Réunions d’abord, les utilisateurs doivent être activés pour [la création de groupes Exchange Online](exchange-teams-interact.md)[, SharePoint Online et OneDrive Entreprise](sharepoint-onedrive-interact.md) et Microsoft 365. Meetings First est pris en charge pour les utilisateurs dont la boîte aux lettres se trouve sur Exchange en local, ou qui n’ont pas de création de groupe SharePoint Online, OneDrive Entreprise ou Microsoft 365. Toutefois, leur expérience sera moins complète. Pour les organisations qui utilisent Exchange Server localement, il peut y avoir (en fonction de la version de Exchange Server) certaines limitations à la création et à l’affichage des réunions à partir du client Teams, ainsi que certaines limitations en ce qui concerne les fonctionnalités de conformité.
...
Toutes les fonctionnalités et expériences utilisateur dans Les réunions d’abord, y compris la configuration du client Teams et la [conformité automatique](teams-client-experience-and-conformance-to-coexistence-modes.md) de l’expérience utilisateur, de Meetings Migration Service et de Better Together, sont configurées en accordant à l’utilisateur (ou au groupe d’utilisateurs ou à la valeur par défaut du locataire) le [mode de coexistence SfBWithTeamsCollabAndMeetings](setting-your-coexistence-and-upgrade-settings.md) dans le [Centre d’administration Microsoft Teams](manage-teams-in-modern-portal.md) ou à l’aide de [PowerShell](/powershell/module/skype/grant-csteamsupgradepolicy?view=skype-ps).

![Capture d’écran des paramètres d’administration pour activer Les réunions d’abord.](media/teams-meeting-admin-settings.png)

Si vous souhaitez masquer l’application Teams et Canaux dans la navigation gauche du client Teams de vos utilisateurs pour concentrer davantage leur expérience sur les réunions, vous pouvez utiliser la [stratégie d’installation de l’application](teams-app-setup-policies.md).
