...
* Le nom de la colonne devient le nom du compartiment. Chaque compartiment que vous spécifiez s’affiche dans la liste compartiments des applications Teams qui utilisent la hiérarchie.
...
Après avoir défini votre hiérarchie dans le fichier de schéma CSV, vous êtes prêt à la télécharger dans Teams. Pour ce faire, exécutez la commande suivante. Vous devez être un administrateur global ou un administrateur de service Teams pour cette étape.
...
Le script suivant peut être utilisé pour créer les équipes et télécharger un fichier .csv dans votre client Microsoft Teams. Si vous avez une hiérarchie existante, ce script la remplace.
