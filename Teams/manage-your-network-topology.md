---
title: Gérer la topologie de votre réseau pour les fonctionnalités vocales cloud dans Microsoft Teams
author: CarolynRowe
ms.author: crowe
manager: serdars
...
ms.locfileid: "69812691"
---
# <a name="manage-your-network-topology-for-cloud-voice-features-in-microsoft-teams"></a>Gérer la topologie de votre réseau pour les fonctionnalités vocales cloud dans Microsoft Teams

Si votre organisation déploie le [routage basé sur la localisation pour le routage direct](location-based-routing-plan.md) ou les [appels d’urgence dynamiques](configure-dynamic-emergency-calling.md), vous devez configurer les paramètres réseau à utiliser avec ces fonctionnalités vocales cloud dans Microsoft Teams. Les paramètres réseau sont utilisés pour déterminer l’emplacement d’un client Teams et incluent les régions réseau, les sites réseau, les sous-réseaux et les adresses IP approuvées. En fonction de la fonctionnalité de voix cloud et de la fonctionnalité que vous déployez, vous configurez tout ou partie de ces paramètres. Pour en savoir plus sur ces termes, consultez [Paramètres réseau pour les fonctionnalités vocales cloud](cloud-voice-network-settings.md).

Vous configurez les paramètres réseau dans la page **Topologie** réseau du Centre d’administration Microsoft Teams ou à l’aide de Windows PowerShell.
...
Vous gérez des adresses IP approuvées externes sous l’onglet **Adresses IP approuvées** de la page **Topologie réseau** du Centre d’administration Microsoft Teams. Vous pouvez ajouter un nombre illimité d’adresses IP externes approuvées.

#### <a name="add-a-trusted-ip-address"></a>Ajouter une adresse IP approuvée

1. Dans le volet de navigation gauche du Centre d’administration Microsoft Teams, accédez à Topologie **réseau** **emplacements** > , puis cliquez sur l’onglet **Adresses IP approuvées**.
...
Pour effectuer les étapes de cette section, vous devez vous familiariser avec les applets de commande PowerShell. Pour plus d’informations, consultez [Vue d’ensemble de Teams PowerShell](teams-powershell-overview.md).

### <a name="define-network-regions"></a>Définir des régions réseau

 Utilisez l’applet de [commande New-CsTenantNetworkRegion](/powershell/module/skype/New-CsTenantNetworkRegion) pour définir des régions réseau. Notez que le paramètre RegionID est un nom logique qui représente la zone géographique de la région et n’a aucune dépendance ou restriction et que le paramètre d’ID&gt; de site CentralSite &lt;est facultatif.
...
Le tableau suivant présente les sites réseau définis dans cet exemple.
...
Dans cet exemple, nous créons une association entre le sous-réseau 192.168.0.0 et le site réseau delhi et entre le sous-réseau 2001:4898:e8:25:844e:926f:85ad:dd8e et le site réseau hyderabad.
...
Le tableau suivant présente les sous-réseaux définis dans cet exemple.
...
Voir aussi [Set-CsTenantNetworkSubnet](/powershell/module/skype/set-cstenantnetworksubnet).
...
Utilisez l’applet de commande [New-CsTenantTrustedIPAddress pour définir des sous-réseaux](/powershell/module/skype/new-cstenanttrustedipaddress?view=skype-ps) externes et les affecter au locataire. Vous pouvez définir un nombre illimité de sous-réseaux externes pour un locataire.
...
## <a name="enabling-network-roaming-policies"></a>Activation des stratégies d’itinérance réseau

Une fois que vous avez configuré vos stratégies d’itinérance réseau, vous devez activer **recherche de configuration réseau** _ dans chacune de vos _ *Stratégies de réunion** dans le Centre Administration Teams sous **Réunions > Stratégies de réunion.**

Vous pouvez modifier la stratégie globale ou créer une stratégie et l’affecter à des utilisateurs spécifiques.
