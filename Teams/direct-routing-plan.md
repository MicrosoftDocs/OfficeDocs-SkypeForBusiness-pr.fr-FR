...
description: Découvrez comment le routage direct Microsoft vous permet de connecter un contrôleur de frontière de session (SBC) fourni par le client pris en charge à Système téléphonique.
ms.openlocfilehash: dd3b9ef6517c1a4c63fe12f7b7eeba4078df8ddd
ms.sourcegitcommit: 2b1290b763c73f64c84c7568b16962e4ae48acf6
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 06/01/2022
ms.locfileid: "65823063"
---
...
Le routage direct vous permet de connecter un contrôleur de frontière de session (SBC) pris en charge et fourni par le client à Système téléphonique. Avec cette fonctionnalité, vous pouvez configurer la connectivité rtc locale avec Microsoft Teams client, comme illustré dans le diagramme suivant : 

![Diagramme montrant la configuration de la connectivité RTC locale.](media/PlanDirectRouting1-PSTNwithTeams.png "Configuration de la connectivité RTC locale avec Microsoft Teams client")
...
- Utilisez pratiquement n’importe quelle jonction RTC avec Système téléphonique. 

- Configurez l’interopérabilité entre les équipements de téléphonie appartenant au client, tels qu’un pbX (Private Branch Exchange) tiers, des périphériques analogiques et des Teams.
...
Le routage direct prend également en charge les utilisateurs disposant de la licence supplémentaire pour le plan d’appel Microsoft. Pour plus d’informations, consultez [Système téléphonique et forfaits d’appels](calling-plan-landing-page.md). 

Avec le routage direct, lorsque les utilisateurs participent à une conférence planifiée, le numéro de connexion est fourni par le service Microsoft Audioconférence, ce qui nécessite une licence appropriée.  Lors de la numérotation, le service Microsoft Audioconférence place l’appel à l’aide des fonctionnalités d’appel en ligne, ce qui nécessite une licence appropriée. (Notez que si un utilisateur n’a pas de licence Microsoft Audioconférence, l’appel est acheminé via le routage direct.) Pour plus d’informations, consultez [Réunions en ligne avec Teams](https://www.microsoft.com/en-us/microsoft-teams/online-meetings). 
...
|Nom de domaine complet (FQDN) pour le SBC|Nom de domaine complet pour le SBC, où la partie domaine du nom de domaine complet est l’un des domaines inscrits dans votre Microsoft 365 ou Office 365 organisation. Pour plus d’informations, consultez [les noms de domaine SBC](#sbc-domain-names).|
...
Adresses IP et ports de pare-feu pour Microsoft Teams média |Pour plus d’informations, consultez [LES URL et plages d’adresses IP](/office365/enterprise/urls-and-ip-address-ranges). |
|||

## <a name="licensing-and-other-requirements"></a>Licences et autres exigences 

Les licences suivantes doivent être attribuées aux utilisateurs du routage direct dans Microsoft 365 : 

- système Téléphone Microsoft
- Microsoft Teams + Skype Entreprise Plan 2, s’il est inclus dans les licences
- Microsoft Audioconférence (Lisez les notes et le paragraphe ci-dessous pour obtenir des exemples spécifiques sur le moment où cette licence est requise.)

> [!NOTE]
> Skype Entreprise plan ne doit pas être supprimé d’un contrat de licence où il est inclus. 
> 
> [!IMPORTANT]
> Cloud de la communauté du secteur public les utilisateurs haute et dod doivent désactiver toute licence Audioconférence incluse dans G5 et attendre d’activer n’importe quel Audioconférence jusqu’à ce que le routage direct ait été entièrement configuré. Les utilisateurs doivent disposer de numéros de téléphone de connexion configurés et d’un pavé de numérotation opérationnel avant d’activer Audioconférence licences. Pour plus d’informations, consultez [Audioconférence avec routage direct pour Cloud de la communauté du secteur public High et DoD](./audio-conferencing-with-direct-routing-for-gcch-and-dod.md).
...
> Pour Cloud de la communauté du secteur public High et DoD, n’affectez pas de licence Audioconférence pour les utilisateurs G5. Pour les utilisateurs G3, n’affectez pas de licence Audioconférence tant que le routage direct n’est pas entièrement configuré et que l’utilisateur dispose d’un pavé de numérotation fonctionnel.


### <a name="ad-hoc-call-escalation-and-audio-conferencing-license"></a>Escalade d’appels ad hoc et licence Audioconférence

Un utilisateur Teams peut démarrer un appel Teams-à-PSTN ou Teams-à-Teams et y ajouter un participant RTC. Ce scénario est appelé conférence ad hoc. Le chemin d’accès de l’appel varie selon que l’utilisateur qui effectue l’escalade de l’appel a ou non une licence Microsoft Audioconférence attribuée :

- **Si une licence Microsoft Audioconférence est attribuée à l’utilisateur Teams qui effectue l’escalade de l’appel**, l’escalade se produit via le service Microsoft Audioconférence. Le participant RTC distant qui est invité à l’appel existant reçoit une notification concernant l’appel entrant et voit le numéro du pont Microsoft affecté à l’utilisateur Teams qui a initié l’escalade.

- **Si la licence Microsoft Audioconférence n’est pas attribuée à l’utilisateur Teams qui effectue l’escalade de l’appel**, l’escalade se produit via un contrôleur de frontière de session connecté à l’interface de routage direct. Le participant RTC distant qui est invité à l’appel reçoit une notification concernant l’appel entrant et voit le numéro de l’utilisateur Teams qui a initié l’escalade. Le SBC spécifique utilisé pour l’escalade est défini par la stratégie de routage de l’utilisateur. 
...
Pour plus d’informations sur les licences Système téléphonique, consultez [Tirer le meilleur parti des options de Office et de](https://products.office.com/compare-all-microsoft-office-products?tab=2) [plan](/office365/servicedescriptions/office-365-platform-service-description/office-365-plan-options) et [Microsoft Teams licences de module complémentaire](./teams-add-on-licensing/microsoft-teams-add-on-licensing.md). 
...
- Téléphones de zone commune. Voir [Configurer les téléphones de zone commune pour Microsoft Teams](./set-up-common-area-phones.md). Vous n’avez pas besoin d’une licence de plan d’appel lors de la configuration d’une Téléphone de zone commune avec routage direct.
...
> [!NOTE]
> Si la prise en charge de Mutual TLS (MTLS) est activée pour la connexion Teams sur le SBC, vous devez installer les certificats Baltimore CyberTrust Root et DigiCert Global Root G2 dans le magasin racine approuvé SBC du contexte TLS Teams. (Cela est dû au fait que les certificats de service Microsoft utilisent l’un de ces deux certificats racines.) Pour télécharger ces certificats racines, consultez [Office 365 chaînes de chiffrement](/microsoft-365/compliance/encryption-office-365-certificate-chains). Pour plus d’informations, consultez [Office modifications de certificat TLS](/microsoft-365/compliance/encryption-office-365-tls-certificates-changes).
...
- Office 365 Cloud de la communauté du secteur public élevé
- Office 365 DoD

En savoir plus sur [les environnements Office 365 et le gouvernement des États-Unis](/office365/servicedescriptions/office-365-platform-service-description/office-365-us-government/office-365-us-government), tels que Cloud de la communauté du secteur public, Cloud de la communauté du secteur public High et DoD.

### <a name="microsoft-365-office-365-and-office-365-gcc-environments"></a>environnements Microsoft 365, Office 365 et Office 365 Cloud de la communauté du secteur public
...
### <a name="office-gcc-dod-environment"></a>environnement DoD Office Cloud de la communauté du secteur public
...
### <a name="office-365-gcc-high-environment"></a>environnement élevé Office 365 Cloud de la communauté du secteur public

Le point de connexion pour le routage direct est le nom de domaine complet suivant :

**sip.pstnhub.gov.teams.microsoft.us** : nom de domaine complet global. Étant donné que l’environnement Cloud de la communauté du secteur public High n’existe que dans les centres de données des États-Unis, il n’existe aucun nom de domaine complet secondaire et tertiaire.
...
|:--- |:--- |:--- |:--- |:--- |
|SIP/TLS|SIP Proxy|SBC|1024 – 65535|Défini sur le SBC (pour Office 365 Cloud de la communauté du secteur public port 5061 élevé/DoD uniquement doit être utilisé)|
...
### <a name="leg-between-sbc-and-cloud-media-processor-or-microsoft-teams-client"></a>Étape entre le processeur SBC et le processeur multimédia cloud ou le client Microsoft Teams

S’applique à la fois aux cas de contournement multimédia et aux cas de non-contournement.

L’interface de routage direct sur la jambe entre le contrôleur de bordure de session et le processeur multimédia cloud (sans contournement multimédia) ou entre le client Teams et le SBC (si le contournement multimédia est activé) peut utiliser les codecs suivants :
...
### <a name="leg-between-microsoft-teams-client-and-cloud-media-processor"></a>Tronçon entre le client Microsoft Teams et le processeur multimédia cloud
...
Sur la jambe entre le processeur multimédia cloud et le client Microsoft Teams, SILK ou G.722 est utilisé. Le choix du codec sur cette étape est basé sur des algorithmes Microsoft, qui prennent en compte plusieurs paramètres. 
...
Les appareils qui ont été validés sont répertoriés comme certifiés pour Teams le routage direct. Les appareils certifiés fonctionnent dans tous les scénarios. 
