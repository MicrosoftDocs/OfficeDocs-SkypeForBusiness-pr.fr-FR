...
|Création d’une <sup>conversation 1, </sup> <sup>2</sup>|    ChatCreated|    Une conversation Teams a été créée.|
...
|Extraire une <sup>conversation 1, </sup> <sup>2</sup>   |ChatRetrieved  |Une conversation Microsoft Teams a été récupérée.|
...
|Lire un message <sup>1, </sup> <sup>2</sup> |MessageRead    |Le message d’une conversation ou d’un canal a été récupéré.|
...
|Notifications de modification de message <sup>1, </sup> <sup>2</sup> |SubscribedToMessages   |Un abonnement a été créé par une application d’écoute qui permet de recevoir les notifications de modification des messages.|
...
|Conversation mise à jour <sup>1, </sup> <sup>2</sup> |ChatUpdated    |Une conversation Teams a été mise à jour.|
|Message mis à jour <sup>(1, </sup> <sup>2)</sup>  |MessageUpdated |Un message de conversation ou de canal a été mis à jour.|
...
Les changements d’appartenance aux Teams (par exemple, utilisateurs ajoutés ou supprimés) via Azure Active Directory (Azure AD), le portail d’administration Microsoft 365 ou l’API Graph Groups Microsoft 365 apparaissent dans les messages d’audit d’Teams et dans le canal Général avec une attribution à du propriétaire existant de l’équipe, et non au véritable initiateur de l’action. Dans ces scénarios, consultez les journaux d’audit d’Azure AD [Microsoft 365 des groupes](/microsoft-365/compliance/search-the-audit-log-in-security-and-compliance) pour consulter les informations pertinentes.
