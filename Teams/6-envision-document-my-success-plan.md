...
Cet article donne une vue d’ensemble des conditions requises pour documenter correctement votre déploiement de voix cloud. En définissant et documentant tous les points de décision et les étapes suivantes lors de la planification de votre déploiement vocal dans le cloud, vous pouvez vous assurer que toutes les parties prenantes et les membres de l’équipe de projet sont alignés pour réussir. 
...
-   **Par site :** Ce modèle convient aux cas où votre organisation est géographiquement dispersée et où les branches comptent un nombre important d’employés. Toutefois, ce modèle de déploiement peut perturber la communication au sein des départements où les employés de département sont répartis sur plusieurs emplacements.

-   **Par division**: ce modèle est généralement la meilleure option pour les entreprises de taille moyenne et garantit que les départements concernés ont la même expérience.

-   **Toute l’entreprise en une fois :** Ce modèle est généralement la meilleure option pour les petites entreprises, où tous les employés obtiennent la même expérience à partir du premier jour du déploiement.
...
|-------------------------------------------------------|--------------|------------------------------|------------|---------------------------------------|-------------------------------|
| Appel de lancement d’une présentation de programme                     | R, A         | C                            |            |                                       |                               |
| Configurer le tableau de bord de qualité des appels                         | I            | C                            | R, A       |                                       |                               |
| Partager le questionnaire de découverte lors d’un appel de lancement | I            | C                            | R, A       |                                       |                               |
| Lancement de la phase de conception                                | R, A         | C                            |            |                                       |                               |
...
Pour exécuter le déploiement voix cloud dans des blocs de travail gérables, nous vous recommandons de créer un plan d’exécution trimestrielle basé sur vos résultats de clé d’objectif (OKR), le modèle de déploiement que vous avez choisi et la fonctionnalité d’exécution de projet de votre organisation.

Vous pourrez ainsi suivre l’avancement sur une base trimestrielle, réviser le plan si nécessaire et déployer les fonctionnalités vocales cloud en fonction de la capacité d’exécution de votre organisation.

Si votre organisation n’a qu’un ou deux sites, vous n’avez peut-être pas besoin d’un plan d’exécution trimestrielle, car vous vous attendriez à être complètement déployé dans un court laps de temps.

Voici un exemple de plan d’exécution trimestrielle pour la phase de conception d’une implémentation vocale dans le cloud.
...
| Autriche : Vienne                          | 500                 | Oui                | Routage direct via le système téléphonique     | T2 CY2018          |
| Italie : Milan                             | 200                 | Oui                | N/A                             | T2 CY2018          |
| Amérique du Sud : Brésil                    | 1500                | Oui                | Routage direct via le système téléphonique     | T2 CY2018          |
| Inde : Contrôle                             | 7000                | Oui                | N/A                             | T3 CY2018          |
...
Indiquez comment vous avez planifié l’exécution du projet afin d’implémenter la solution dans votre organisation.

### <a name="adoption-plan"></a>Plan d’adoption

Une fois que vous avez effectué l’évaluation de la préparation pour l’adoption, l’équipe de projet doit créer un ensemble complet de plans de communication, un plan de formation et des plans pour les activités d’adoption avant le lancement, le lancement et après le lancement.
...
<tr><td><img src="media/audio_conferencing_image7.png" alt="An icon depicting decision points"/> <br/>Points de décision</td><td><ul><li>Décidez comment documenter l’ensemble de votre plan de réussite pour la livraison de vos charges de travail vocales dans le cloud.</li></ul></td></tr>
<tr><td><img src="media/audio_conferencing_image9.png" alt="An icon depicting the next steps"/><br/>Étapes suivantes</td><td><ul><li>Confirmez que tous les composants de votre plan de réussite ont été documentés.</li><li>Agréger des composants individuels de votre plan de réussite dans un document récapitulatif unique (facultatif).</li></ul></td></tr>
</table>
