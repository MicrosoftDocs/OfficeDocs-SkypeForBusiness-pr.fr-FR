...
Cet article explique comment configurer un contrôleur de session en bordure (SBC) et le connecter au routage direct du système téléphonique.  Voici l’étape 1 des étapes suivantes pour configurer le routage direct :

- **Étape 1. Connecter votre SBC à Phone System et valider la connexion** (cet article)
...
2. [Connectez le SBC au client.](#connect-the-sbc-to-the-tenant)
...
Vous pouvez utiliser une session PowerShell connectée au client pour associer le SBC à l’interface de routage direct. Pour ouvrir une session PowerShell, suivez les étapes décrites dans [configurer votre ordinateur pour une Windows PowerShell.](https://docs.microsoft.com/SkypeForBusiness/set-up-your-computer-for-windows-powershell/set-up-your-computer-for-windows-powershell)
...
Utilisez la [cmdlet New-CsOnlinePSTNGateway](https://docs.microsoft.com/powershell/module/skype/new-csonlinepstngateway) pour connecter le SBC au client. Dans une session PowerShell, tapez ce qui suit, puis appuyez sur Entrée :
...
> [!NOTE]
> Cet exemple montre uniquement les paramètres minimum requis. Vous pouvez définir d’autres paramètres avec la cmdlet [New-CsOnlinePSTNGateway](https://docs.microsoft.com/powershell/module/skype/new-csonlinepstngateway) pendant le processus de connexion. Pour en savoir plus, [consultez les paramètres SBC.](#sbc-settings)
...
Après avoir connecté le SBC, utilisez l’cmdlet [Get-CsOnlinePSTNGateway](https://docs.microsoft.com/powershell/module/skype/get-csonlinepstngateway) pour vérifier qu’il figure dans la liste des SBC couplés. Tapez ce qui suit dans une session Remote PowerShell, puis appuyez sur Entrée :
...
Ce tableau répertorie les options que vous pouvez définir pour le SBC dans le Centre d’administration Microsoft Teams et à l’aide de la cmdlet [New-CsOnlinePSTNGateway.](https://docs.microsoft.com/powershell/module/skype/new-csonlinepstngateway)
...
|Non|**Activé**|Activé|Utilisez cette fonction pour activer le SBC pour les appels sortants. Vous pouvez l’utiliser pour supprimer temporairement le SBC du service pendant la mise à jour ou pendant la maintenance. |False|Vrai<br/>False|Boolean|
...
|Non|**Faire sonner le téléphone en essayant de trouver l’utilisateur**|GenerateRingingWhileLocatingUser|Définissez si un signal audio est envoyé à l’appelant pour indiquer que Teams est en train d’établir l’appel. Ce paramètre s’applique uniquement au routage direct en mode de dérivation non multimédia. Parfois, les appels entrants du PSTN vers les clients Teams peuvent prendre plus de temps que prévu pour être établis. Dans ce cas, il se peut que l’appelant n’entende rien, que le client Teams ne sonne pas et que l’appel soit annulé par certains fournisseurs de télécommunications. Ce paramètre permet d’éviter les silences inattendus qui peuvent se produire dans ces scénarios.|Vrai|Vrai<br/>False|Boolean|
|Non| - |MediaBypass|Ce paramètre indique si le paramètre SBC prend en charge la dérivation média et si vous souhaitez l’utiliser pour ce paramètre. |Aucun|Vrai<br/>False|Boolean|
