...
Teams prend également en charge plusieurs configurations dans les environnements virtuels. Il s’agit de modes VDI, dédiés, partagés, persistants et non persistants. Les fonctionnalités sont en développement continu et sont régulièrement ajoutées, et elles seront étendues au fil du temps.
...
Actuellement, Teams sur VDI avec l’optimisation audio/vidéo (AV) est certifiée avec Azure Virtual Desktop, Citrix et VMware. Examinez les informations de cette section pour vous assurer que vous respectez toutes les conditions requises pour utiliser les fonctionnalités adéquates.
...
## <a name="install-or-update-the-teams-desktop-app-on-vdi"></a>Installer ou mettre à jour Teams’application de bureau en environnement VDI
...
Pour la plupart des déploiements VDI, nous vous recommandons de déployer Teams l’installation par ordinateur. Pour mettre à jour vers la dernière version Teams, commencez par la procédure de désinstallation suivie de la dernière Teams de version.
...
Teams via Applications Microsoft 365 pour les grandes entreprises est installé par utilisateur. Pour en savoir plus, consultez la section Installer ou mettre à jour [Teams’application de bureau sur VDI](#install-or-update-the-teams-desktop-app-on-vdi).
...
Teams est également ajouté aux installations existantes d’Applications Microsoft 365 pour les grandes entreprises. Étant donné Applications Microsoft 365 pour les grandes entreprises est installée Teams utilisateur uniquement, consultez la section Installer ou mettre à jour l’application Teams [bureau sur VDI](#install-or-update-the-teams-desktop-app-on-vdi).
...
1. Téléchargez le Teams package MSI qui correspond à votre système d’exploitation VDI VM à l’aide de l’un des liens suivants :
    ...
    > [!NOTE]
    > Pour les clouds du gouvernement, voir Installer en bloc Teams à l’aide Windows [Installer (MSI)](msi-deployment.md) pour consulter les liens de téléchargement vers les fichiers MSI.
...
2. Installez le MSI sur la VDI VM en exécutant l’une des commandes suivantes :
    ...
    - Installation par ordinateur
        ...
        Ce processus ajoute une clé de Registre requise à l’ordinateur, qui permet au Teams de s’installer qu’il s’agit d’une instance VDI.  Sans ce dernier, le programme d’installation échouera, en indiquant : « L’installation a échoué.  Installation impossible pour tous les utilisateurs lorsqu’un environnement VDI n’est pas détecté. »
...
- **Dépendances :** il peut s’agir de dépendances en relation avec l’infrastructure, la charge de travail et d’autres considérations environnementales en dehors de Teams’application de bureau.
...
### <a name="migrate-from-skype-for-business-on-vdi-to-teams-on-vdi"></a>Migrer d Skype Entreprise d’un Teams VDI vers un Teams VDI

Si vous migrez de Skype Entreprise sur VDI vers Teams sur un VDI, outre les différences entre les deux applications, il existe quelques différences lorsque VDI est également implémenté. Certaines fonctionnalités qui ne sont pas actuellement pris en charge dans Teams VDI qui sont dans Skype Entreprise VDI sont les suivantes :
...
Vous pouvez définir des stratégies à l’aide du Teams d’administration ou de PowerShell. La propagation des modifications de stratégie peut prendre quelques heures. Si vous ne voyez pas immédiatement de modifications pour un compte donné, essayez à nouveau dans quelques heures.
...
#### <a name="assign-policies-using-the-teams-admin-center"></a>Attribuer des stratégies à l’aide Teams d’administration
...
## <a name="migrate-teams-on-vdi-with-chat-and-collaboration-to-optimize-teams-with-calling-and-meetings"></a>Migrer des Teams en VDI avec la conversation et la collaboration afin d’optimiser Teams avec les appels et les réunions
...
Teams stratégies VDI sont disponibles dans le module Teams ressources. Ces stratégies sont actives et appliquées sur des environnements VDI non optimisés.
...
Lorsque les utilisateurs dont la stratégie VDI `DisableCallsAndMeetings` est définie `$true` pour se Teams sur un VDI, ils ne peuvent pas :
...
Lorsque les utilisateurs pour qui une stratégie VDI `DisableAudioVideoInCallsAndMeetings` est définie pour `$true` se Teams une VDI, ils doivent :
...
- Boutons HID et contrôles LED entre l’Teams et les appareils
...
- Prenez le contrôle :
  ...
  - Pris en charge pendant PowerPoint session de partage.

Pour Teams problèmes connus qui ne sont pas liés à une Teams [VDI, consultez le support technique de votre organisation](/MicrosoftTeams/troubleshoot/teams-welcome).

## <a name="troubleshooting"></a>Résolution des problèmes

### <a name="troubleshoot-citrix-components"></a>Résoudre les problèmes des composants Citrix

#### <a name="teams-crashes-or-the-teams-sign-in-screen-is-blank"></a>Teams incidents ou l’écran Teams se connecte est vide
