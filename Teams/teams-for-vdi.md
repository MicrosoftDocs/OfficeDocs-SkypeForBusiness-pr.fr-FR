...
|ScreenSharingMode | Détermine le mode dans lequel un utilisateur est autorisé à partager son écran dans des appels ou des réunions. | Définissez sur Disabled pour empêcher l’utilisateur de partager son écran. |
...
| AllowOutlookAddIn |Détermine si un utilisateur peut planifier des réunions d’équipes dans le client de bureau Outlook. | Valeur définie sur false pour interdire à un utilisateur de planifier des réunions d’équipes dans le client de bureau Outlook. |
...
| AllowExternalParticipantGiveRequestControl | Détermine si les participants externes peuvent demander ou donner le contrôle du partage d’écran. | Valeur définie sur false pour interdire à un utilisateur externe de donner et de demander le contrôle pendant une réunion. |
| AllowPowerPointSharing |Détermine si le partage PowerPoint est autorisé dans les réunions d’un utilisateur. |Valeur définie sur false pour empêcher un utilisateur de partager des fichiers PowerPoint dans une réunion. |
...
| AllowTranscription |Détermine si les légendes et les transcriptions en temps réel et/ou après réunion sont autorisées dans les réunions d’un utilisateur. | Affectez la valeur false pour interdire la transcription et les légendes dans une réunion. |
| AllowSharedNotes | Détermine si les utilisateurs sont autorisés à prendre des notes partagées. | Valeur définie sur false pour interdire aux utilisateurs de suivre des notes partagées. |
| MediaBitRateKB |Détermine le taux de bits média pour les transmissions du partage audio/vidéo/d’application dans les réunions. | La valeur suggérée est 5000 (5 Mo). Vous pouvez le modifier en fonction des besoins de votre organisation. |

#### <a name="create-and-assign-a-meeting-policy"></a>Création et affectation d’une stratégie de réunion

1. Démarrez une session Windows PowerShell en tant qu’administrateur.
1. Connectez-vous au connecteur en ligne Skype.
...
1. Afficher une liste des options de stratégie de réunion

      ```powershell
      Get-CsTeamsMeetingPolicy
      ```

1. Recherchez l’option de stratégie prédéfinie dans laquelle toutes les stratégies de réunion sont désactivées. Elle se présente comme suit.
...
1. Appliquez l’option de stratégie intégrée à AllOff à tous les utilisateurs qui utiliseront teams dans un environnement virtualisé.
...
> [!NOTE]
> Les conditions suivantes s’appliquent à l’application de bureau teams et à l’application Web Teams.
...
1. Exécutez la commande suivante pour installer le MSI sur la machine virtuelle VDI (ou terminer la mise à jour).
...
1. Exécutez la commande suivante pour désinstaller le MSI de l’ordinateur virtuel VDI (ou préparer la mise à jour).
